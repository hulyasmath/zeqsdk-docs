---
sidebar_position: 7
title: "HULYAS Strict Solver"
description: "Auto-tunes β parameter iteratively until error ≤ 0.1% or max iterations. The 7-Step Wizard"
---
...
Auto-tunes β parameter iteratively until error ≤ 0.1% or max iterations. The 7-Step Wizard It belongs to the **ODE solver, master equation** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/framework/solve/strict`, Zeq runs the **HULYAS Strict Solver** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **HULYAS Strict Solver** when you need a ode solver, master equation primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/framework/solve/strict` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/framework/solve/strict \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/framework/solve/strict", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/framework/solve/strict", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/framework/solve/strict"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/framework/solve/strict");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/framework/solve/strict");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/framework/solve/strict")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/framework/solve/strict",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/framework/solve/strict", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/framework/solve/strict
...
Every response from `/api/framework/solve/strict` carries:
