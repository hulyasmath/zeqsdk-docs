...
sidebar_position: 1
title: "ZeqCompute"
description: "Core computation — select domain, resolve operators, compute R(t), build master equation, return ZeqState with ZeqProof HMAC. Supports algebraic, ODE, and strict modes."
---
...
Reach for **ZeqCompute** when you need a ode solver, master equation primitive that:
...
- **Must scale across domains** — the same endpoint works whether you're driving one call per minute or part of a high-throughput pipeline (subject to rate limit 60/min + daily plan limit)
