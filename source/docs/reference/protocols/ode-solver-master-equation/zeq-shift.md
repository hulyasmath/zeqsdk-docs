...
sidebar_position: 5
title: "ZeqShift"
description: "Time-series R(t) projection forward 1–64 Zeqond steps. Shows phase, R_t, and delta per step."
---
...
Reach for **ZeqShift** when you need a ode solver, master equation primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/zeq/shift` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/zeq/shift", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/shift"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/shift");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/shift");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/shift")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/shift",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/shift", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/zeq/shift
...
Every response from `/api/zeq/shift` carries:
