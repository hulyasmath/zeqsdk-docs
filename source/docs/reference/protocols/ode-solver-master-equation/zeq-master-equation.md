---
sidebar_position: 8
title: "Master Equation Builder"
description: "Builds the synthesized HULYAS master equation for a given domain and operator set without executing the solver. Returns the full equation string and term breakdown."
---
...
| **Category** | ODE solver, master equation |
| **Version** | 1.0 |
...
Builds the synthesized HULYAS master equation for a given domain and operator set without executing the solver. Returns the full equation string and term breakdown. It belongs to the **ODE solver, master equation** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/zeq/master-equation`, Zeq runs the **Master Equation Builder** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its ode solver, master equation operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **Master Equation Builder** when you need a ode solver, master equation primitive that:
...
If you only need a one-shot ode solver, master equation answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/zeq/master-equation` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/zeq/master-equation \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/master-equation", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/zeq/master-equation", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/master-equation"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/master-equation");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/master-equation");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/master-equation")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/master-equation",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/master-equation", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/zeq/master-equation
...
Every response from `/api/zeq/master-equation` carries:
...
- All protocols in this family — see the [ODE solver, master equation](/reference/protocols) category
