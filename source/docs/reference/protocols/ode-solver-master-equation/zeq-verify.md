---
sidebar_position: 2
title: "ZeqProof Verify"
description: "Verify a ZeqProof HMAC-SHA256 binding. Confirms a previous computation"
---
...
Reach for **ZeqProof Verify** when you need a ode solver, master equation primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/zeq/verify` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/zeq/verify", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/verify"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/verify");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/verify");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/verify")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/verify",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/verify", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/zeq/verify
...
Every response from `/api/zeq/verify` carries:
