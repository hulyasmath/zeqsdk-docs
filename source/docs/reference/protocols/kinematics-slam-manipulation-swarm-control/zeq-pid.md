...
sidebar_position: 5
title: "ZeqPID"
description: "PID controller tuning with HulyaPulse sampling. Auto-tune via Ziegler-Nichols on Zeqond timestep, bode plot analysis, stability margin computation."
---
...
Reach for **ZeqPID** when you need a kinematics, slam, manipulation, swarm, control primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/robotics/pid` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/robotics/pid \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/robotics/pid", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/robotics/pid", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/robotics/pid"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/robotics/pid");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/robotics/pid");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/robotics/pid")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/robotics/pid",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/robotics/pid", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/robotics/pid
...
Every response from `/api/robotics/pid` carries:
