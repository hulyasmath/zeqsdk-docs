...
sidebar_position: 1
title: "ZeqKinematics"
description: "Forward/inverse kinematics with HulyaPulse timing. DH parameter chains up to 12-DOF, singularity avoidance, workspace analysis with R(t) joint trajectory smoothing."
---
...
Reach for **ZeqKinematics** when you need a kinematics, slam, manipulation, swarm, control primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/robotics/kinematics` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/robotics/kinematics \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/robotics/kinematics", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/robotics/kinematics", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/robotics/kinematics"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/robotics/kinematics");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/robotics/kinematics");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/robotics/kinematics")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/robotics/kinematics",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/robotics/kinematics", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/robotics/kinematics
...
Every response from `/api/robotics/kinematics` carries:
