...
sidebar_position: 4
title: "ZeqSwarm"
description: "Multi-robot swarm coordination via HulyaPulse synchronization. Formation control, task allocation, consensus algorithms — all agents tick on the same Zeqond clock."
---
...
Reach for **ZeqSwarm** when you need a kinematics, slam, manipulation, swarm, control primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/robotics/swarm` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/robotics/swarm \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/robotics/swarm", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/robotics/swarm", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/robotics/swarm"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/robotics/swarm");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/robotics/swarm");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/robotics/swarm")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/robotics/swarm",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/robotics/swarm", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/robotics/swarm
...
Every response from `/api/robotics/swarm` carries:
