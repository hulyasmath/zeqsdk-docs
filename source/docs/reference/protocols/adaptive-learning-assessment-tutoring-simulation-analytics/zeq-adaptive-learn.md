---
sidebar_position: 1
title: "ZeqAdaptiveLearning"
description: "Adaptive learning engine using R(t)-modulated knowledge state estimation. Item response theory (IRT) with Zeqond-paced mastery gates, spaced repetition scheduling."
---
...
Reach for **ZeqAdaptiveLearning** when you need a adaptive learning, assessment, tutoring, simulation, analytics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/edu/adaptive` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/edu/adaptive \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/edu/adaptive", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/edu/adaptive", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/edu/adaptive"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/edu/adaptive");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/edu/adaptive");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/edu/adaptive")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/edu/adaptive",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/edu/adaptive", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/edu/adaptive
...
Every response from `/api/edu/adaptive` carries:
