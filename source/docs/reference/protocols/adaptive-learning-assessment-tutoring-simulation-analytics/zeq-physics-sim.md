---
sidebar_position: 3
title: "ZeqPhysicsSimulation"
description: "Interactive physics simulation for education. Pendulums, projectiles, orbits, wave tanks — all Zeqond-stepped with R(t) accuracy for classroom demonstrations."
---
...
| **Category** | Adaptive Learning, Assessment, Tutoring, Simulation, Analytics |
| **Version** | 1.287.0 |
...
Interactive physics simulation for education. Pendulums, projectiles, orbits, wave tanks — all Zeqond-stepped with R(t) accuracy for classroom demonstrations. It belongs to the **Adaptive Learning, Assessment, Tutoring, Simulation, Analytics** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/edu/physics-sim`, Zeq runs the **ZeqPhysicsSimulation** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its adaptive learning, assessment, tutoring, simulation, analytics operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqPhysicsSimulation** when you need a adaptive learning, assessment, tutoring, simulation, analytics primitive that:
...
If you only need a one-shot adaptive learning, assessment, tutoring, simulation, analytics answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/edu/physics-sim` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/edu/physics-sim \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/edu/physics-sim", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/edu/physics-sim", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/edu/physics-sim"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/edu/physics-sim");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/edu/physics-sim");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/edu/physics-sim")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/edu/physics-sim",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/edu/physics-sim", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/edu/physics-sim
...
Every response from `/api/edu/physics-sim` carries:
...
- All protocols in this family — see the [Adaptive Learning, Assessment, Tutoring, Simulation, Analytics](/reference/protocols) category
