...
sidebar_position: 4
title: "ZeqTutor"
description: "Socratic tutoring protocol. Misconception detection, scaffolded hints, knowledge graph traversal — paced at Zeqond intervals for cognitive load management."
---
...
Reach for **ZeqTutor** when you need a adaptive learning, assessment, tutoring, simulation, analytics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/edu/tutor` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/edu/tutor \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/edu/tutor", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/edu/tutor", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/edu/tutor"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/edu/tutor");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/edu/tutor");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/edu/tutor")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/edu/tutor",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/edu/tutor", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/edu/tutor
...
Every response from `/api/edu/tutor` carries:
