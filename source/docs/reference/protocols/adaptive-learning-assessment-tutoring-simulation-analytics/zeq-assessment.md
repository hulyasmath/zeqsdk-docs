...
sidebar_position: 2
title: "ZeqAssessment"
description: "Automated assessment generation and scoring. Bloom"
---
...
Reach for **ZeqAssessment** when you need a adaptive learning, assessment, tutoring, simulation, analytics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/edu/assessment` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/edu/assessment \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/edu/assessment", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/edu/assessment", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/edu/assessment"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/edu/assessment");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/edu/assessment");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/edu/assessment")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/edu/assessment",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/edu/assessment", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/edu/assessment
...
Every response from `/api/edu/assessment` carries:
