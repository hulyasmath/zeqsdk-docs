---
sidebar_position: 5
title: "ZeqLearningAnalytics"
description: "Learning analytics dashboard data. Engagement metrics, at-risk prediction, cohort analysis with HulyaPulse temporal patterns for early intervention."
---
...
Reach for **ZeqLearningAnalytics** when you need a adaptive learning, assessment, tutoring, simulation, analytics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/edu/analytics` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/edu/analytics \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/edu/analytics", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/edu/analytics", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/edu/analytics"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/edu/analytics");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/edu/analytics");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/edu/analytics")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/edu/analytics",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/edu/analytics", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/edu/analytics
...
Every response from `/api/edu/analytics` carries:
