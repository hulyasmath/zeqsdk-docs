---
sidebar_position: 1
title: "ZeqVitals — Biosignal Processing"
description: "Biosignal processing synchronized to HulyaPulse. ECG, EEG, SpO2 waveforms aligned to the 1.287 Hz grid — universal temporal reference for all physiological data."
---
...
Reach for **ZeqVitals — Biosignal Processing** when you need a biosignals, pharma, diagnostics, genomics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/health/vitals/process` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/health/vitals/process \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/health/vitals/process", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/health/vitals/process", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/health/vitals/process"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/health/vitals/process");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/health/vitals/process");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/health/vitals/process")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/health/vitals/process",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/health/vitals/process", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/health/vitals/process
...
Every response from `/api/health/vitals/process` carries:
