---
sidebar_position: 7
title: "ZeqEpi — Epidemic Modeling"
description: "SIR/SEIR epidemic modeling via ODE solver with HulyaPulse temporal resolution. Model disease spread with real-time R(t)-modulated transmission rates."
---
...
Reach for **ZeqEpi — Epidemic Modeling** when you need a biosignals, pharma, diagnostics, genomics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/health/epidemic/simulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/health/epidemic/simulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/health/epidemic/simulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/health/epidemic/simulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/health/epidemic/simulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/health/epidemic/simulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/health/epidemic/simulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/health/epidemic/simulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/health/epidemic/simulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/health/epidemic/simulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/health/epidemic/simulate
...
Every response from `/api/health/epidemic/simulate` carries:
