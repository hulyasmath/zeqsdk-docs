---
sidebar_position: 6
title: "ZeqHeart — Cardiac Modeling"
description: "Cardiac electrophysiology modeling using coupled oscillator equations synced to HulyaPulse. Simulates action potential propagation and arrhythmia detection."
---
...
Reach for **ZeqHeart — Cardiac Modeling** when you need a biosignals, pharma, diagnostics, genomics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/health/heart/model` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/health/heart/model \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/health/heart/model", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/health/heart/model", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/health/heart/model"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/health/heart/model");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/health/heart/model");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/health/heart/model")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/health/heart/model",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/health/heart/model", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/health/heart/model
...
Every response from `/api/health/heart/model` carries:
