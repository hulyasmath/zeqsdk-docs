---
sidebar_position: 5
title: "ZeqTrial — Clinical Data Integrity"
description: "Clinical trial data integrity with ZeqProof-chained observations. Every data point carries a cryptographic proof binding it to the exact Zeqond it was recorded — tamper-evident by "
---
...
Reach for **ZeqTrial — Clinical Data Integrity** when you need a biosignals, pharma, diagnostics, genomics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/health/trial/record` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/health/trial/record \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/health/trial/record", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/health/trial/record", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/health/trial/record"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/health/trial/record");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/health/trial/record");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/health/trial/record")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/health/trial/record",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/health/trial/record", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/health/trial/record
...
Every response from `/api/health/trial/record` carries:
