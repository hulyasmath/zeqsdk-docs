---
sidebar_position: 2
title: "ZeqPharma — Pharmacokinetics"
description: "Drug absorption, distribution, metabolism, and excretion modeling via the HULYAS ODE solver. Compartmental PK models integrated with RK4 on the Zeqond timebase."
---
...
Reach for **ZeqPharma — Pharmacokinetics** when you need a biosignals, pharma, diagnostics, genomics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/health/pharma/simulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/health/pharma/simulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/health/pharma/simulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/health/pharma/simulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/health/pharma/simulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/health/pharma/simulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/health/pharma/simulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/health/pharma/simulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/health/pharma/simulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/health/pharma/simulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/health/pharma/simulate
...
Every response from `/api/health/pharma/simulate` carries:
