---
sidebar_position: 3
title: "ZeqDiagnostic — Pattern Recognition"
description: "Cross-domain pattern recognition using operator coupling. Feed in multi-modal biosignal data — the framework identifies anomalies by coherence deviation from healthy baselines."
---
...
Cross-domain pattern recognition using operator coupling. Feed in multi-modal biosignal data — the framework identifies anomalies by coherence deviation from healthy baselines. It belongs to the **Biosignals, pharma, diagnostics, genomics** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/health/diagnostic/analyze`, Zeq runs the **ZeqDiagnostic — Pattern Recognition** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqDiagnostic — Pattern Recognition** when you need a biosignals, pharma, diagnostics, genomics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/health/diagnostic/analyze` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/health/diagnostic/analyze \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/health/diagnostic/analyze", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/health/diagnostic/analyze", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/health/diagnostic/analyze"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/health/diagnostic/analyze");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/health/diagnostic/analyze");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/health/diagnostic/analyze")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/health/diagnostic/analyze",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/health/diagnostic/analyze", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/health/diagnostic/analyze
...
Every response from `/api/health/diagnostic/analyze` carries:
