---
sidebar_position: 2
title: "ZeqDesalination"
description: "Desalination plant optimization. Reverse osmosis membrane modeling, energy recovery, brine management with R(t)-modulated pressure control."
---
...
| **Category** | Purification, Desalination, Flood, Groundwater, Wastewater |
| **Version** | 1.287.0 |
...
Reach for **ZeqDesalination** when you need a purification, desalination, flood, groundwater, wastewater primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/water/desalination` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/water/desalination \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "feedSalinity_ppm": 1,
  "targetCapacity_m3_day": 1,
...
```javascript
const res = await fetch("https://www.zeq.dev/api/water/desalination", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/water/desalination", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/water/desalination"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/water/desalination");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/water/desalination");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/water/desalination")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/water/desalination",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/water/desalination", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/water/desalination
...
Every response from `/api/water/desalination` carries:
...
- All protocols in this family — see the [Purification, Desalination, Flood, Groundwater, Wastewater](/reference/protocols) category
