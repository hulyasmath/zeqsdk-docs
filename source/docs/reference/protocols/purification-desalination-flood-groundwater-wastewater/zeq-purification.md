---
sidebar_position: 1
title: "ZeqPurification"
description: "Water purification process optimization. Coagulation/flocculation dosing, filtration rate, UV disinfection with HulyaPulse CT value monitoring (C×T)."
---
...
Reach for **ZeqPurification** when you need a purification, desalination, flood, groundwater, wastewater primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/water/purification` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/water/purification \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/water/purification", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/water/purification", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/water/purification"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/water/purification");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/water/purification");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/water/purification")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/water/purification",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/water/purification", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/water/purification
...
Every response from `/api/water/purification` carries:
