---
sidebar_position: 4
title: "ZeqGroundwater"
description: "Groundwater flow modeling. Darcy"
---
...
Reach for **ZeqGroundwater** when you need a purification, desalination, flood, groundwater, wastewater primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/water/groundwater` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/water/groundwater \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/water/groundwater", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/water/groundwater", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/water/groundwater"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/water/groundwater");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/water/groundwater");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/water/groundwater")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/water/groundwater",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/water/groundwater", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/water/groundwater
...
Every response from `/api/water/groundwater` carries:
