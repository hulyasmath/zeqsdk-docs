...
sidebar_position: 5
title: "ZeqWastewater"
description: "Wastewater treatment optimization. Activated sludge modeling (ASM1), nutrient removal (N/P), biogas production, effluent quality prediction with Zeqond process control."
---
...
Reach for **ZeqWastewater** when you need a purification, desalination, flood, groundwater, wastewater primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/water/wastewater` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/water/wastewater \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/water/wastewater", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/water/wastewater", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/water/wastewater"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/water/wastewater");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/water/wastewater");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/water/wastewater")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/water/wastewater",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/water/wastewater", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/water/wastewater
...
Every response from `/api/water/wastewater` carries:
