---
sidebar_position: 1
title: "ZeqStressAnalysis"
description: "FEA stress analysis with R(t)-modulated loading. Von Mises, Tresca, principal stresses, safety factor computation. Linear and nonlinear static solvers."
---
...
Reach for **ZeqStressAnalysis** when you need a stress, composites, nano, corrosion, 3d print primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/materials/stress` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/materials/stress \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/materials/stress", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/materials/stress", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/materials/stress"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/materials/stress");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/materials/stress");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/materials/stress")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/materials/stress",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/materials/stress", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/materials/stress
...
Every response from `/api/materials/stress` carries:
