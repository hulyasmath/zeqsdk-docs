...
sidebar_position: 3
title: "ZeqNano"
description: "Nanomaterial property prediction. Quantum confinement effects, surface-to-volume ratios, bandgap tuning with R(t) size distribution modeling."
---
...
Nanomaterial property prediction. Quantum confinement effects, surface-to-volume ratios, bandgap tuning with R(t) size distribution modeling. It belongs to the **Stress, Composites, Nano, Corrosion, 3D Print** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/materials/nano`, Zeq runs the **ZeqNano** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqNano** when you need a stress, composites, nano, corrosion, 3d print primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/materials/nano` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/materials/nano \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/materials/nano", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/materials/nano", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/materials/nano"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/materials/nano");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/materials/nano");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/materials/nano")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/materials/nano",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/materials/nano", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/materials/nano
...
Every response from `/api/materials/nano` carries:
