...
sidebar_position: 2
title: "ZeqComposite"
description: "Composite laminate analysis. Classical lamination theory (CLT) with HulyaPulse failure criteria (Tsai-Wu, Hashin). Ply-by-ply stress, first-ply failure prediction."
---
...
Reach for **ZeqComposite** when you need a stress, composites, nano, corrosion, 3d print primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/materials/composite` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/materials/composite \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/materials/composite", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/materials/composite", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/materials/composite"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/materials/composite");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/materials/composite");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/materials/composite")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/materials/composite",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/materials/composite", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/materials/composite
...
Every response from `/api/materials/composite` carries:
