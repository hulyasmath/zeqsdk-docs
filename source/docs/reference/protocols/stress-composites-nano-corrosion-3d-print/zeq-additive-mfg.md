---
sidebar_position: 5
title: "ZeqAdditiveMfg"
description: "3D printing / additive manufacturing optimization. Layer-by-layer thermal simulation, support structure generation, distortion prediction with R(t) cooling models."
---
...
| **Category** | Stress, Composites, Nano, Corrosion, 3D Print |
| **Version** | 1.287.0 |
...
3D printing / additive manufacturing optimization. Layer-by-layer thermal simulation, support structure generation, distortion prediction with R(t) cooling models. It belongs to the **Stress, Composites, Nano, Corrosion, 3D Print** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/materials/additive`, Zeq runs the **ZeqAdditiveMfg** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its stress, composites, nano, corrosion, 3d print operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqAdditiveMfg** when you need a stress, composites, nano, corrosion, 3d print primitive that:
...
If you only need a one-shot stress, composites, nano, corrosion, 3d print answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/materials/additive` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/materials/additive \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/materials/additive", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/materials/additive", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/materials/additive"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/materials/additive");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/materials/additive");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/materials/additive")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/materials/additive",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/materials/additive", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/materials/additive
...
Every response from `/api/materials/additive` carries:
...
- All protocols in this family — see the [Stress, Composites, Nano, Corrosion, 3D Print](/reference/protocols) category
