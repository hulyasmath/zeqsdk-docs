---
sidebar_position: 5
title: "ZeqMesh Identity — Federated"
description: "Federated identity across Zeq OS instances. No central authority — your equation produces the same ZID on ANY node. Mesh verification via ZeqLattice coherence."
---

# ZeqMesh Identity — Federated

> Federated identity across Zeq OS instances. No central authority — your equation produces the same ZID on ANY node. Mesh verification via ZeqLattice coherence.
...
Reach for **ZeqMesh Identity — Federated** when you need a zeq auth, equation-based identity primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/auth/mesh/verify` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/auth/mesh/verify \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auth/mesh/verify", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/auth/mesh/verify", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auth/mesh/verify"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auth/mesh/verify");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auth/mesh/verify");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auth/mesh/verify")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/auth/mesh/verify",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auth/mesh/verify", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/auth/mesh/verify
...
Every response from `/api/auth/mesh/verify` carries:
