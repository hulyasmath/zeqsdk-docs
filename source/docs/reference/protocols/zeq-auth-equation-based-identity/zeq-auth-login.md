---
sidebar_position: 2
title: "Zeq Auth — Login"
description: "Re-enter your equation to authenticate. Same equation → same evaluation → same hash → same ZID. No password database to breach. Infinite mathematical function space."
---
...
Reach for **Zeq Auth — Login** when you need a zeq auth, equation-based identity primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/auth/login` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/auth/login \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auth/login", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/auth/login", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auth/login"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auth/login");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auth/login");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auth/login")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/auth/login",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auth/login", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/auth/login
...
Every response from `/api/auth/login` carries:
