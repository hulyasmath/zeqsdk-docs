---
sidebar_position: 6
title: "ZeqReputation — Mathematical Reputation"
description: "Reputation score derived from computation history. More verified computations with lower error = higher reputation. Score is a function of ZeqProof chain length and precision."
---
...
Reach for **ZeqReputation — Mathematical Reputation** when you need a zeq auth, equation-based identity primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/auth/reputation` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/auth/reputation \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auth/reputation", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/auth/reputation", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auth/reputation"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auth/reputation");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auth/reputation");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auth/reputation")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/auth/reputation",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auth/reputation", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/auth/reputation
...
Every response from `/api/auth/reputation` carries:
