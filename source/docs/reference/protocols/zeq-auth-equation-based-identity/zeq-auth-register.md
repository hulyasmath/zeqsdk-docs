---
sidebar_position: 1
title: "Zeq Auth — Register"
description: "Your equation IS your identity. Enter a mathematical equation — it"
---
...
Reach for **Zeq Auth — Register** when you need a zeq auth, equation-based identity primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/auth/register` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/auth/register \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auth/register", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/auth/register", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auth/register"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auth/register");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auth/register");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auth/register")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/auth/register",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auth/register", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/auth/register
...
Every response from `/api/auth/register` carries:
