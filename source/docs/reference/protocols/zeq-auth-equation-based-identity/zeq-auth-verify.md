---
sidebar_position: 3
title: "Zeq Auth — Verify Token"
description: "Verify a Zeq Auth token. Returns validity, ZID, and display name if valid."
---
...
Reach for **Zeq Auth — Verify Token** when you need a zeq auth, equation-based identity primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/auth/verify` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/auth/verify \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auth/verify", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/auth/verify", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auth/verify"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auth/verify");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auth/verify");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auth/verify")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/auth/verify",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auth/verify", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/auth/verify
...
Every response from `/api/auth/verify` carries:
