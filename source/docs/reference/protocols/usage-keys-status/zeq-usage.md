...
sidebar_position: 1
title: "API Usage"
description: "Current API usage — calls used today, daily limit, remaining, plan tier."
---
...
Reach for **API Usage** when you need a usage, keys, status primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/zeq/usage` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/zeq/usage \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/usage", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/zeq/usage", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/usage"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/usage");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/usage");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/usage")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/usage",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/usage", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/zeq/usage
...
Every response from `/api/zeq/usage` carries:
