---
sidebar_position: 2
title: "ZeqField Status"
description: "Cryptographic infrastructure status — cipher, key derivation, salt source, proof algorithm. No key material exposed."
---
...
Cryptographic infrastructure status — cipher, key derivation, salt source, proof algorithm. No key material exposed. It belongs to the **Usage, keys, status** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/admin/zeqfield/status`, Zeq runs the **ZeqField Status** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its usage, keys, status operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqField Status** when you need a usage, keys, status primitive that:
...
If you only need a one-shot usage, keys, status answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `GET` request to `/api/admin/zeqfield/status` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/admin/zeqfield/status \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/admin/zeqfield/status", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/admin/zeqfield/status", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/admin/zeqfield/status"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/admin/zeqfield/status");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/admin/zeqfield/status");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/admin/zeqfield/status")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/admin/zeqfield/status",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/admin/zeqfield/status", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/admin/zeqfield/status
...
Every response from `/api/admin/zeqfield/status` carries:
...
- All protocols in this family — see the [Usage, keys, status](/reference/protocols) category
