---
sidebar_position: 9
title: "ZeqEntangle — Entanglement Analysis"
description: "Entanglement analysis and Bell state preparation. Compute concurrence, von Neumann entropy, and Bell inequality violations using quantum operators QM3, QM4."
---
...
Reach for **ZeqEntangle — Entanglement Analysis** when you need a orbital, trajectory, radiation, comms primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/quantum/entangle/analyze` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/quantum/entangle/analyze \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/quantum/entangle/analyze", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/quantum/entangle/analyze", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/quantum/entangle/analyze"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/quantum/entangle/analyze");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/quantum/entangle/analyze");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/quantum/entangle/analyze")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/quantum/entangle/analyze",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/quantum/entangle/analyze", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/quantum/entangle/analyze
...
Every response from `/api/quantum/entangle/analyze` carries:
