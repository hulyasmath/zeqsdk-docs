---
sidebar_position: 1
title: "ZeqOrbit — Orbital Mechanics"
description: "Orbital mechanics computation using gravitational physics operators. Hohmann transfers, orbital elements, period, velocity — all synced to Zeqond time."
---
...
Orbital mechanics computation using gravitational physics operators. Hohmann transfers, orbital elements, period, velocity — all synced to Zeqond time. It belongs to the **Orbital, trajectory, radiation, comms** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/space/orbit/compute`, Zeq runs the **ZeqOrbit — Orbital Mechanics** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqOrbit — Orbital Mechanics** when you need a orbital, trajectory, radiation, comms primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/space/orbit/compute` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/space/orbit/compute \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/space/orbit/compute", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/space/orbit/compute", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/space/orbit/compute"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/space/orbit/compute");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/space/orbit/compute");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/space/orbit/compute")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/space/orbit/compute",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/space/orbit/compute", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/space/orbit/compute
...
Every response from `/api/space/orbit/compute` carries:
