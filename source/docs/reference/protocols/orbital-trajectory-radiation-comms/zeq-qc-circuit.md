---
sidebar_position: 7
title: "ZeqQuantum — Circuit Simulation"
description: "Quantum circuit simulation using quantum mechanics operators. Simulate gates (H, CNOT, T, S, Rx, Ry, Rz) on up to 20 qubits with HulyaPulse-synced decoherence."
---
...
Reach for **ZeqQuantum — Circuit Simulation** when you need a orbital, trajectory, radiation, comms primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/quantum/circuit/simulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/quantum/circuit/simulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/quantum/circuit/simulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/quantum/circuit/simulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/quantum/circuit/simulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/quantum/circuit/simulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/quantum/circuit/simulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/quantum/circuit/simulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/quantum/circuit/simulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/quantum/circuit/simulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/quantum/circuit/simulate
...
Every response from `/api/quantum/circuit/simulate` carries:
