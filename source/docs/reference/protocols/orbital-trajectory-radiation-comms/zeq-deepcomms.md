---
sidebar_position: 5
title: "ZeqDeepComms — Deep Space Communication"
description: "Deep space communication with phase-locked signal recovery. Compensates for light-delay using Zeqond prediction — receiver knows the exact phase the signal was sent at."
---
...
Reach for **ZeqDeepComms — Deep Space Communication** when you need a orbital, trajectory, radiation, comms primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/space/comms/link` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/space/comms/link \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/space/comms/link", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/space/comms/link", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/space/comms/link"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/space/comms/link");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/space/comms/link");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/space/comms/link")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/space/comms/link",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/space/comms/link", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/space/comms/link
...
Every response from `/api/space/comms/link` carries:
