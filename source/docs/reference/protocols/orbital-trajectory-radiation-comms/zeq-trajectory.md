---
sidebar_position: 2
title: "ZeqTrajectory — Multi-Body Optimization"
description: "Multi-body trajectory optimization using RK4 solver with gravitational operator coupling. Compute optimal transfer orbits between planets."
---
...
Multi-body trajectory optimization using RK4 solver with gravitational operator coupling. Compute optimal transfer orbits between planets. It belongs to the **Orbital, trajectory, radiation, comms** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/space/trajectory/optimize`, Zeq runs the **ZeqTrajectory — Multi-Body Optimization** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqTrajectory — Multi-Body Optimization** when you need a orbital, trajectory, radiation, comms primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/space/trajectory/optimize` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/space/trajectory/optimize \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/space/trajectory/optimize", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/space/trajectory/optimize", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/space/trajectory/optimize"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/space/trajectory/optimize");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/space/trajectory/optimize");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/space/trajectory/optimize")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/space/trajectory/optimize",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/space/trajectory/optimize", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/space/trajectory/optimize
...
Every response from `/api/space/trajectory/optimize` carries:
