---
sidebar_position: 3
title: "ZeqRadiation — Space Radiation"
description: "Space radiation exposure calculation using particle physics operators. GCR, SPE, and trapped radiation belt dose estimation for crew and electronics."
---
...
Reach for **ZeqRadiation — Space Radiation** when you need a orbital, trajectory, radiation, comms primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/space/radiation/calculate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/space/radiation/calculate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/space/radiation/calculate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/space/radiation/calculate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/space/radiation/calculate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/space/radiation/calculate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/space/radiation/calculate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/space/radiation/calculate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/space/radiation/calculate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/space/radiation/calculate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/space/radiation/calculate
...
Every response from `/api/space/radiation/calculate` carries:
