---
sidebar_position: 11
title: "ZeqVQE — Variational Eigensolver"
description: "Variational Quantum Eigensolver for molecular ground state energy. Parameterized circuits optimized classically, evaluated quantumly — hybrid quantum-classical on HulyaPulse."
---
...
Reach for **ZeqVQE — Variational Eigensolver** when you need a orbital, trajectory, radiation, comms primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/quantum/vqe` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/quantum/vqe \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/quantum/vqe", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/quantum/vqe", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/quantum/vqe"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/quantum/vqe");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/quantum/vqe");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/quantum/vqe")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/quantum/vqe",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/quantum/vqe", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/quantum/vqe
...
Every response from `/api/quantum/vqe` carries:
