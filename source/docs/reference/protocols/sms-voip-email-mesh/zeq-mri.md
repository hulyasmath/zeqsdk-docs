...
sidebar_position: 6
title: "ZeqMRI"
description: "HulyaPulse-synced MRI signal reconstruction. R(t) modulation on k-space trajectories for artifact suppression. Supports T1/T2/FLAIR sequences with Zeqond-gated cardiac sync."
---
...
Reach for **ZeqMRI** when you need a sms, voip, email, mesh primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/medical/mri` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/medical/mri \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/medical/mri", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/medical/mri", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/medical/mri"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/medical/mri");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/medical/mri");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/medical/mri")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/medical/mri",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/medical/mri", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/medical/mri
...
Every response from `/api/medical/mri` carries:
