...
sidebar_position: 9
title: "ZeqX-Ray"
description: "Digital X-ray enhancement with pulse-modulated contrast. Adaptive histogram equalization synced to Zeqond intervals for consistent exposure across body regions."
---
...
Digital X-ray enhancement with pulse-modulated contrast. Adaptive histogram equalization synced to Zeqond intervals for consistent exposure across body regions. It belongs to the **SMS, VoIP, Email, Mesh** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/medical/xray`, Zeq runs the **ZeqX-Ray** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqX-Ray** when you need a sms, voip, email, mesh primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/medical/xray` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/medical/xray \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/medical/xray", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/medical/xray", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/medical/xray"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/medical/xray");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/medical/xray");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/medical/xray")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/medical/xray",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/medical/xray", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/medical/xray
...
Every response from `/api/medical/xray` carries:
