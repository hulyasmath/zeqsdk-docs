...
sidebar_position: 11
title: "ZeqDialysis"
description: "Hemodialysis flow optimization via HulyaPulse. R(t)-modulated blood pump rates for hemodynamic stability. Real-time urea kinetics (Kt/V) with Zeqond sampling."
---
...
Hemodialysis flow optimization via HulyaPulse. R(t)-modulated blood pump rates for hemodynamic stability. Real-time urea kinetics (Kt/V) with Zeqond sampling. It belongs to the **SMS, VoIP, Email, Mesh** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/medical/dialysis`, Zeq runs the **ZeqDialysis** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqDialysis** when you need a sms, voip, email, mesh primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/medical/dialysis` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/medical/dialysis \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "patientWeight_kg": 1,
  "targetKtV": 1,
  "bloodFlowRate_mlMin": 1,
  "dialysateFlow_mlMin": 1
}'
```
...
```javascript
const res = await fetch("https://www.zeq.dev/api/medical/dialysis", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/medical/dialysis", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/medical/dialysis"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/medical/dialysis");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/medical/dialysis");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/medical/dialysis")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/medical/dialysis",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/medical/dialysis", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/medical/dialysis
...
Every response from `/api/medical/dialysis` carries:
