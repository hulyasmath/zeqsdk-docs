...
sidebar_position: 15
title: "ZeqEPS"
description: "Electrophysiology study protocol. HulyaPulse-gated intracardiac signal analysis for arrhythmia mapping, conduction velocity measurement, and ablation targeting."
---
...
Electrophysiology study protocol. HulyaPulse-gated intracardiac signal analysis for arrhythmia mapping, conduction velocity measurement, and ablation targeting. It belongs to the **SMS, VoIP, Email, Mesh** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/medical/eps`, Zeq runs the **ZeqEPS** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqEPS** when you need a sms, voip, email, mesh primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/medical/eps` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/medical/eps \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "intracardiacSignals": "<intracardiacSignals>",
...
```javascript
const res = await fetch("https://www.zeq.dev/api/medical/eps", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/medical/eps", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/medical/eps"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/medical/eps");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/medical/eps");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/medical/eps")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/medical/eps",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/medical/eps", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/medical/eps
...
Every response from `/api/medical/eps` carries:
