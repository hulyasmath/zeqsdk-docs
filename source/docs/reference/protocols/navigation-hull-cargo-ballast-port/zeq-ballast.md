...
sidebar_position: 4
title: "ZeqBallast"
description: "Ballast water management with HulyaPulse exchange timing. BWM Convention compliance, invasive species risk, stability during exchange operations."
---
...
Ballast water management with HulyaPulse exchange timing. BWM Convention compliance, invasive species risk, stability during exchange operations. It belongs to the **Navigation, Hull, Cargo, Ballast, Port** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/maritime/ballast`, Zeq runs the **ZeqBallast** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqBallast** when you need a navigation, hull, cargo, ballast, port primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/maritime/ballast` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/maritime/ballast \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "currentBallast": "<currentBallast>",
...
```javascript
const res = await fetch("https://www.zeq.dev/api/maritime/ballast", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/maritime/ballast", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/maritime/ballast"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/maritime/ballast");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/maritime/ballast");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/maritime/ballast")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/maritime/ballast",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/maritime/ballast", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/maritime/ballast
...
Every response from `/api/maritime/ballast` carries:
