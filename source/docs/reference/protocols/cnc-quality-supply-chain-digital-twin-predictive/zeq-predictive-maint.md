---
sidebar_position: 5
title: "ZeqPredictiveMaintenance"
description: "Predictive maintenance via HulyaPulse vibration spectral analysis. Bearing fault detection, remaining useful life estimation, maintenance scheduling optimization."
---
...
| **Category** | CNC, Quality, Supply Chain, Digital Twin, Predictive |
| **Version** | 1.287.0 |
...
Predictive maintenance via HulyaPulse vibration spectral analysis. Bearing fault detection, remaining useful life estimation, maintenance scheduling optimization. It belongs to the **CNC, Quality, Supply Chain, Digital Twin, Predictive** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/mfg/predictive-maintenance`, Zeq runs the **ZeqPredictiveMaintenance** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its cnc, quality, supply chain, digital twin, predictive operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqPredictiveMaintenance** when you need a cnc, quality, supply chain, digital twin, predictive primitive that:
...
If you only need a one-shot cnc, quality, supply chain, digital twin, predictive answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/mfg/predictive-maintenance` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/mfg/predictive-maintenance \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/mfg/predictive-maintenance", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/mfg/predictive-maintenance", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/mfg/predictive-maintenance"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/mfg/predictive-maintenance");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/mfg/predictive-maintenance");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/mfg/predictive-maintenance")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/mfg/predictive-maintenance",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/mfg/predictive-maintenance", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/mfg/predictive-maintenance
...
Every response from `/api/mfg/predictive-maintenance` carries:
...
- All protocols in this family — see the [CNC, Quality, Supply Chain, Digital Twin, Predictive](/reference/protocols) category
