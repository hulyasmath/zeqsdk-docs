---
sidebar_position: 3
title: "ZeqSupplyChain"
description: "Supply chain optimization. Demand forecasting, inventory optimization (EOQ/safety stock), logistics routing — all Zeqond-synchronized for just-in-time precision."
---
...
Reach for **ZeqSupplyChain** when you need a cnc, quality, supply chain, digital twin, predictive primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/mfg/supply-chain` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/mfg/supply-chain \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/mfg/supply-chain", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/mfg/supply-chain", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/mfg/supply-chain"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/mfg/supply-chain");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/mfg/supply-chain");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/mfg/supply-chain")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/mfg/supply-chain",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/mfg/supply-chain", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/mfg/supply-chain
...
Every response from `/api/mfg/supply-chain` carries:
