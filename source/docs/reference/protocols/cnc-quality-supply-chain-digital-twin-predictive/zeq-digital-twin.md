---
sidebar_position: 4
title: "ZeqDigitalTwin"
description: "Factory digital twin synchronization. Real-time virtual factory state mirroring at Zeqond intervals, what-if simulation, downtime prediction."
---
...
Reach for **ZeqDigitalTwin** when you need a cnc, quality, supply chain, digital twin, predictive primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/mfg/digital-twin` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/mfg/digital-twin \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/mfg/digital-twin", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/mfg/digital-twin", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/mfg/digital-twin"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/mfg/digital-twin");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/mfg/digital-twin");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/mfg/digital-twin")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/mfg/digital-twin",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/mfg/digital-twin", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/mfg/digital-twin
...
Every response from `/api/mfg/digital-twin` carries:
