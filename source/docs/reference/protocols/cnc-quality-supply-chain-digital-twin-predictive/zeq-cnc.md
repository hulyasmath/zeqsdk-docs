...
sidebar_position: 1
title: "ZeqCNC"
description: "CNC toolpath optimization with Zeqond-quantized feed rates. Chatter detection via R(t) vibration analysis, adaptive feed control, tool wear prediction."
---
...
Reach for **ZeqCNC** when you need a cnc, quality, supply chain, digital twin, predictive primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/mfg/cnc` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```javascript
const res = await fetch("https://www.zeq.dev/api/mfg/cnc", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/mfg/cnc", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/mfg/cnc"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/mfg/cnc");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/mfg/cnc");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/mfg/cnc")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/mfg/cnc",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/mfg/cnc", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/mfg/cnc
...
Every response from `/api/mfg/cnc` carries:
