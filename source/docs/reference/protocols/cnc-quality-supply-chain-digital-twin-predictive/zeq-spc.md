...
sidebar_position: 2
title: "ZeqSPC"
description: "Statistical process control with HulyaPulse sampling intervals. Xbar-R charts, Cp/Cpk, Western Electric rules — Zeqond-stamped control limits for audit traceability."
---
...
Reach for **ZeqSPC** when you need a cnc, quality, supply chain, digital twin, predictive primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/mfg/spc` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```javascript
const res = await fetch("https://www.zeq.dev/api/mfg/spc", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/mfg/spc", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/mfg/spc"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/mfg/spc");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/mfg/spc");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/mfg/spc")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/mfg/spc",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/mfg/spc", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/mfg/spc
...
Every response from `/api/mfg/spc` carries:
