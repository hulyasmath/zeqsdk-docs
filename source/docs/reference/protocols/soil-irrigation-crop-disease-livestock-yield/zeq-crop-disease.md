---
sidebar_position: 3
title: "ZeqCropDisease"
description: "Crop disease detection and prediction. Spectral index analysis (NDVI/NDRE) with HulyaPulse phenology tracking, pathogen risk modeling from humidity/temperature."
---
...
Reach for **ZeqCropDisease** when you need a soil, irrigation, crop disease, livestock, yield primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/agri/disease` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/agri/disease \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/agri/disease", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/agri/disease", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/agri/disease"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/agri/disease");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/agri/disease");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/agri/disease")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/agri/disease",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/agri/disease", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/agri/disease
...
Every response from `/api/agri/disease` carries:
