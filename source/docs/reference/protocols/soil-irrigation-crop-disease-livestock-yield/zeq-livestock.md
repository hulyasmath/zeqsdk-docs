...
sidebar_position: 4
title: "ZeqLivestock"
description: "Livestock health monitoring. Activity patterns, rumination cycles, heat detection, body condition scoring with HulyaPulse circadian alignment."
---
...
Reach for **ZeqLivestock** when you need a soil, irrigation, crop disease, livestock, yield primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/agri/livestock` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/agri/livestock \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/agri/livestock", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/agri/livestock", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/agri/livestock"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/agri/livestock");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/agri/livestock");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/agri/livestock")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/agri/livestock",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/agri/livestock", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/agri/livestock
...
Every response from `/api/agri/livestock` carries:
