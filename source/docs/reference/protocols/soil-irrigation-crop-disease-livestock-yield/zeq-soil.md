...
sidebar_position: 1
title: "ZeqSoil"
description: "Soil composition and nutrient analysis. NPK levels, pH mapping, organic matter content, microbial activity index — all HulyaPulse-timestamped for temporal tracking."
---
...
Reach for **ZeqSoil** when you need a soil, irrigation, crop disease, livestock, yield primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/agri/soil` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/agri/soil \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/agri/soil", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/agri/soil", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/agri/soil"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/agri/soil");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/agri/soil");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/agri/soil")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/agri/soil",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/agri/soil", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/agri/soil
...
Every response from `/api/agri/soil` carries:
