...
description: "HULYAS Information-Theoretic Encryption — AES-256-GCM with PBKDF2-SHA256 key derived from HulyaPulse constants. Includes Landauer certificate."
---
...
HULYAS Information-Theoretic Encryption — AES-256-GCM with PBKDF2-SHA256 key derived from HulyaPulse constants. Includes Landauer certificate. It belongs to the **Encryption, channels, proofs, firewall** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/hite/encrypt`, Zeq runs the **HITE Encryption** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **HITE Encryption** when you need a encryption, channels, proofs, firewall primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/hite/encrypt` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hite/encrypt \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hite/encrypt", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hite/encrypt", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hite/encrypt"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hite/encrypt");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hite/encrypt");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hite/encrypt")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hite/encrypt",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hite/encrypt", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hite/encrypt
...
Every response from `/api/hite/encrypt` carries:
