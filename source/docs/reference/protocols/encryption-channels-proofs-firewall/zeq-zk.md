---
sidebar_position: 4
title: "ZeqZK — Zero-Knowledge Proof"
description: "Zero-knowledge proof using HULYAS field evaluation. Prove you know an equation that produces a specific R(t) value without revealing the equation. Based on the Zeq Auth principle: "
---
...
Reach for **ZeqZK — Zero-Knowledge Proof** when you need a encryption, channels, proofs, firewall primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/security/zk/prove` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/security/zk/prove \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/security/zk/prove", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/security/zk/prove", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/security/zk/prove"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/security/zk/prove");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/security/zk/prove");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/security/zk/prove")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/security/zk/prove",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/security/zk/prove", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/security/zk/prove
...
Every response from `/api/security/zk/prove` carries:
