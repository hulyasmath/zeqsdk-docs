---
sidebar_position: 7
title: "ZeqFirewall — Phase-Locked ACL"
description: "Phase-locked access control. Requests only valid within specific Zeqond windows. Define time-gated permissions that open and close on the HulyaPulse cycle."
---
...
Reach for **ZeqFirewall — Phase-Locked ACL** when you need a encryption, channels, proofs, firewall primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/security/firewall/check` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/security/firewall/check \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/security/firewall/check", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/security/firewall/check", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/security/firewall/check"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/security/firewall/check");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/security/firewall/check");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/security/firewall/check")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/security/firewall/check",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/security/firewall/check", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/security/firewall/check
...
Every response from `/api/security/firewall/check` carries:
