---
sidebar_position: 5
title: "ZeqCert — Mathematical Certificate"
description: "Mathematical certificate authority. Certificates derived from operator coupling strength — not PKI trees. A cert is a ZeqProof chain binding identity to a set of verified computati"
---
...
| **Category** | Encryption, channels, proofs, firewall |
| **Version** | 1.0 |
...
Mathematical certificate authority. Certificates derived from operator coupling strength — not PKI trees. A cert is a ZeqProof chain binding identity to a set of verified computations. It belongs to the **Encryption, channels, proofs, firewall** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/security/cert/issue`, Zeq runs the **ZeqCert — Mathematical Certificate** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its encryption, channels, proofs, firewall operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqCert — Mathematical Certificate** when you need a encryption, channels, proofs, firewall primitive that:
...
If you only need a one-shot encryption, channels, proofs, firewall answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/security/cert/issue` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/security/cert/issue \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/security/cert/issue", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/security/cert/issue", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/security/cert/issue"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/security/cert/issue");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/security/cert/issue");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/security/cert/issue")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/security/cert/issue",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/security/cert/issue", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/security/cert/issue
...
Every response from `/api/security/cert/issue` carries:
...
- All protocols in this family — see the [Encryption, channels, proofs, firewall](/reference/protocols) category
