---
sidebar_position: 6
title: "ZeqKeyEx — Temporal Key Exchange"
description: "Diffie-Hellman key exchange over the Zeqond grid. Both parties derive a shared secret synchronized to the same HulyaPulse phase — temporal key agreement."
---
...
Reach for **ZeqKeyEx — Temporal Key Exchange** when you need a encryption, channels, proofs, firewall primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/security/keyex/init` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/security/keyex/init \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/security/keyex/init", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/security/keyex/init", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/security/keyex/init"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/security/keyex/init");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/security/keyex/init");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/security/keyex/init")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/security/keyex/init",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/security/keyex/init", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/security/keyex/init
...
Every response from `/api/security/keyex/init` carries:
