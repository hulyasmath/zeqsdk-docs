---
sidebar_position: 2
title: "HITE Decryption"
description: "Decrypt HITE-encrypted data. GCM authentication tag verified — integrity guaranteed."
---
...
Reach for **HITE Decryption** when you need a encryption, channels, proofs, firewall primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/hite/decrypt` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hite/decrypt \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hite/decrypt", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hite/decrypt", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hite/decrypt"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hite/decrypt");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hite/decrypt");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hite/decrypt")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hite/decrypt",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hite/decrypt", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hite/decrypt
...
Every response from `/api/hite/decrypt` carries:
