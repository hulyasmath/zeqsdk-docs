---
sidebar_position: 8
title: "ZeqAudit — Computation Audit Trail"
description: "Immutable computation audit trail. Every ZeqProof is chained — each proof references the previous one, creating an unforgeable computation history."
---
...
Immutable computation audit trail. Every ZeqProof is chained — each proof references the previous one, creating an unforgeable computation history. It belongs to the **Encryption, channels, proofs, firewall** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/security/audit/chain`, Zeq runs the **ZeqAudit — Computation Audit Trail** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqAudit — Computation Audit Trail** when you need a encryption, channels, proofs, firewall primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/security/audit/chain` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/security/audit/chain \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/security/audit/chain", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/security/audit/chain", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/security/audit/chain"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/security/audit/chain");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/security/audit/chain");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/security/audit/chain")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/security/audit/chain",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/security/audit/chain", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/security/audit/chain
...
Every response from `/api/security/audit/chain` carries:
