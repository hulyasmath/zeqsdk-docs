...
sidebar_position: 4
title: "ZeqSupplement"
description: "Supplement interaction and optimization. Nutrient absorption timing, synergies (vitamin D + K2), antagonisms (calcium vs iron), bioavailability forms."
---
...
Reach for **ZeqSupplement** when you need a meal planning, macros, allergy, supplement, metabolic primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/nutrition/supplement` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nutrition/supplement \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nutrition/supplement", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nutrition/supplement", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nutrition/supplement"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nutrition/supplement");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nutrition/supplement");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nutrition/supplement")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nutrition/supplement",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nutrition/supplement", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nutrition/supplement
...
Every response from `/api/nutrition/supplement` carries:
