...
sidebar_position: 3
title: "ZeqAllergy"
description: "Food allergen detection and cross-contamination risk assessment. FDA Big 9 allergen scanning, alternative ingredient suggestions, restaurant safety scoring."
---
...
Reach for **ZeqAllergy** when you need a meal planning, macros, allergy, supplement, metabolic primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/nutrition/allergy` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nutrition/allergy \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nutrition/allergy", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nutrition/allergy", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nutrition/allergy"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nutrition/allergy");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nutrition/allergy");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nutrition/allergy")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nutrition/allergy",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nutrition/allergy", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nutrition/allergy
...
Every response from `/api/nutrition/allergy` carries:
