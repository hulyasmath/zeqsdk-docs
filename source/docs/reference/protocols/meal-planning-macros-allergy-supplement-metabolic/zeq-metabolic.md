...
sidebar_position: 5
title: "ZeqMetabolic"
description: "Metabolic rate and body composition modeling. Indirect calorimetry estimation, RQ analysis, lean mass prediction with R(t) circadian metabolic cycling."
---
...
Reach for **ZeqMetabolic** when you need a meal planning, macros, allergy, supplement, metabolic primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/nutrition/metabolic` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nutrition/metabolic \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nutrition/metabolic", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nutrition/metabolic", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nutrition/metabolic"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nutrition/metabolic");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nutrition/metabolic");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nutrition/metabolic")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nutrition/metabolic",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nutrition/metabolic", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nutrition/metabolic
...
Every response from `/api/nutrition/metabolic` carries:
