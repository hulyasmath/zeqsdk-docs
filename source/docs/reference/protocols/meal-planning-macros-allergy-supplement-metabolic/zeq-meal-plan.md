...
sidebar_position: 1
title: "ZeqMealPlan"
description: "Personalized meal planning with HulyaPulse circadian nutrition timing. Macro/micro nutrient targets, meal prep scheduling, grocery list optimization."
---
...
Reach for **ZeqMealPlan** when you need a meal planning, macros, allergy, supplement, metabolic primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/nutrition/meal-plan` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nutrition/meal-plan \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nutrition/meal-plan", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nutrition/meal-plan", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nutrition/meal-plan"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nutrition/meal-plan");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nutrition/meal-plan");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nutrition/meal-plan")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nutrition/meal-plan",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nutrition/meal-plan", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nutrition/meal-plan
...
Every response from `/api/nutrition/meal-plan` carries:
