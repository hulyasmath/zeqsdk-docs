...
sidebar_position: 1
title: "ZeqText"
description: "TESC-encrypted messaging protocol. End-to-end AES-256-GCM with Phase-Locked Authentication Tags (PLATs) that expire every 0.777s. TESC UID serves as your phone number. WebSocket re"
---
...
| **Category** | ZeqText, Globe, ZeqCompress, HulyaForensics |
| **Version** | 1.287.0 |
...
TESC-encrypted messaging protocol. End-to-end AES-256-GCM with Phase-Locked Authentication Tags (PLATs) that expire every 0.777s. TESC UID serves as your phone number. WebSocket real-time delivery, typing indicators, read receipts, offline message queue, entanglement chain replay protection. It belongs to the **ZeqText, Globe, ZeqCompress, HulyaForensics** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/tesc/send`, Zeq runs the **ZeqText** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its zeqtext, globe, zeqcompress, hulyaforensics operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqText** when you need a zeqtext, globe, zeqcompress, hulyaforensics primitive that:
...
If you only need a one-shot zeqtext, globe, zeqcompress, hulyaforensics answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/tesc/send` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/tesc/send \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/tesc/send", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/tesc/send", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/tesc/send"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/tesc/send");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/tesc/send");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/tesc/send")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/tesc/send",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/tesc/send", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/tesc/send
...
Every response from `/api/tesc/send` carries:
...
- All protocols in this family — see the [ZeqText, Globe, ZeqCompress, HulyaForensics](/reference/protocols) category
