---
sidebar_position: 7
title: "HulyaForensics Score"
description: "Quick HF composite score — returns the final S_forensic value from all 20 HF equations with weighted average. Weights: HF5 (legal)=0.20, HF16 (severity)=0.20, HF20 (composite)=0.20"
---
...
Reach for **HulyaForensics Score** when you need a zeqtext, globe, zeqcompress, hulyaforensics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/hf/score` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hf/score \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hf/score", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hf/score", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hf/score"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hf/score");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hf/score");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hf/score")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hf/score",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hf/score", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hf/score
...
Every response from `/api/hf/score` carries:
