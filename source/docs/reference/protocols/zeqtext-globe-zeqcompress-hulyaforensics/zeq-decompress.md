...
sidebar_position: 4
title: "ZeqDecompress"
description: "Decompress .zeqc archives. Validates ZEQC magic header, extracts Zeqond metadata, Zstd decompresses, verifies SHA-256 integrity. Returns original files with creation timestamp and "
---
...
Reach for **ZeqDecompress** when you need a zeqtext, globe, zeqcompress, hulyaforensics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/zeq/decompress` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/zeq/decompress \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/decompress", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/zeq/decompress", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/decompress"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/decompress");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/decompress");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/decompress")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/decompress",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/decompress", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/zeq/decompress
...
Every response from `/api/zeq/decompress` carries:
