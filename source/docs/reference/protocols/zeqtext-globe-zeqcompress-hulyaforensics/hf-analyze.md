---
sidebar_position: 6
title: "HulyaForensics Analyze"
description: "HULYA Forensics — 20-equation pulse-synced forensic analysis. Evaluates: accuracy (HF1), manipulation detection (HF2), smear detection (HF3), source verification (HF4), legal crite"
---
...
Reach for **HulyaForensics Analyze** when you need a zeqtext, globe, zeqcompress, hulyaforensics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/hf/analyze` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hf/analyze \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hf/analyze", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hf/analyze", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hf/analyze"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hf/analyze");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hf/analyze");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hf/analyze")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hf/analyze",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hf/analyze", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hf/analyze
...
Every response from `/api/hf/analyze` carries:
