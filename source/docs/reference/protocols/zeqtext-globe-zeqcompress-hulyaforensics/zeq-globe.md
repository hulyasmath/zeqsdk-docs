...
sidebar_position: 2
title: "Zeq Globe"
description: "Real-time 3D Earth visualization protocol. Serves USGS earthquake data (M2.5+, 60s refresh), Open-Meteo weather for 20+ cities, solar terminator, and UDPDS phi tension disaster pre"
---
...
Reach for **Zeq Globe** when you need a zeqtext, globe, zeqcompress, hulyaforensics primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/zeq/globe` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/zeq/globe \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/globe", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/zeq/globe", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/globe"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/globe");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/globe");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/globe")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/globe",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/globe", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/zeq/globe
...
Every response from `/api/zeq/globe` carries:
