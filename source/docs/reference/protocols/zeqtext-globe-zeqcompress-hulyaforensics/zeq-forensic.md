...
sidebar_position: 5
title: "ZeqForensic"
description: "Data forensic entropy analysis. Computes Shannon entropy, byte frequency distribution, SHA-256 hash, and HulyaPulse sync state. Classifies data as RAW (<6.0 bits/byte), COMPRESSED "
---
...
Reach for **ZeqForensic** when you need a zeqtext, globe, zeqcompress, hulyaforensics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/zeq/forensic` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/zeq/forensic \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/forensic", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/zeq/forensic", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/forensic"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/forensic");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/forensic");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/forensic")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/forensic",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/forensic", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/zeq/forensic
...
Every response from `/api/zeq/forensic` carries:
