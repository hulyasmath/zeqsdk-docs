...
sidebar_position: 3
title: "ZeqCompress"
description: "Zeqond-timestamped compression protocol. Creates .zeqc archives using Zstd level-19 compression with SHA-256 integrity verification. Custom binary format: ZEQC magic header, Zeqond"
---
...
Reach for **ZeqCompress** when you need a zeqtext, globe, zeqcompress, hulyaforensics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/zeq/compress` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/zeq/compress \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/compress", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/zeq/compress", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/compress"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/compress");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/compress");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/compress")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/compress",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/compress", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/zeq/compress
...
Every response from `/api/zeq/compress` carries:
