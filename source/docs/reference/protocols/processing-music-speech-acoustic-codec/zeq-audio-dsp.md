...
sidebar_position: 1
title: "ZeqAudioDSP"
description: "Audio DSP with HulyaPulse-synced sample clock. FFT, filtering (low/high/band/notch), compression, EQ — all phase-aligned to Zeqond boundaries for zero-drift processing."
---
...
Reach for **ZeqAudioDSP** when you need a processing, music, speech, acoustic, codec primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/audio/dsp` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/audio/dsp \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/audio/dsp", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/audio/dsp", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/audio/dsp"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/audio/dsp");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/audio/dsp");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/audio/dsp")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/audio/dsp",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/audio/dsp", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/audio/dsp
...
Every response from `/api/audio/dsp` carries:
