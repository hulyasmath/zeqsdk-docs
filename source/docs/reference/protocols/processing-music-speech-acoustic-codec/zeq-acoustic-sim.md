---
sidebar_position: 4
title: "ZeqAcousticSim"
description: "Room acoustics simulation. Ray tracing, RT60 calculation, speech intelligibility (STI), concert hall optimization with R(t) impulse response modeling."
---
...
Reach for **ZeqAcousticSim** when you need a processing, music, speech, acoustic, codec primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/audio/acoustic` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/audio/acoustic \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/audio/acoustic", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/audio/acoustic", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/audio/acoustic"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/audio/acoustic");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/audio/acoustic");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/audio/acoustic")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/audio/acoustic",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/audio/acoustic", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/audio/acoustic
...
Every response from `/api/audio/acoustic` carries:
