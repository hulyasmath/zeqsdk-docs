...
sidebar_position: 5
title: "ZeqCodec"
description: "Audio codec with HulyaPulse quantization. Zeqond-aligned frame encoding, perceptual masking model, variable bitrate targeting MUSHRA ≥85 at minimal bandwidth."
---
...
Reach for **ZeqCodec** when you need a processing, music, speech, acoustic, codec primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/audio/codec` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/audio/codec \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "audioData": "<audioData>",
  "targetBitrate_kbps": 1,
  "mode": "<mode>"
}'
```
...
```javascript
const res = await fetch("https://www.zeq.dev/api/audio/codec", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/audio/codec", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/audio/codec"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/audio/codec");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/audio/codec");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/audio/codec")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/audio/codec",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/audio/codec", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/audio/codec
...
Every response from `/api/audio/codec` carries:
