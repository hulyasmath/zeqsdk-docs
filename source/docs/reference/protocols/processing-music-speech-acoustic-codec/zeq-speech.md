...
sidebar_position: 3
title: "ZeqSpeech"
description: "Speech processing with HulyaPulse phoneme alignment. Voice activity detection, speaker diarization, formant analysis with R(t) pitch tracking at 1.287 Hz resolution."
---
...
Reach for **ZeqSpeech** when you need a processing, music, speech, acoustic, codec primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/audio/speech` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/audio/speech \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/audio/speech", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/audio/speech", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/audio/speech"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/audio/speech");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/audio/speech");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/audio/speech")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/audio/speech",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/audio/speech", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/audio/speech
...
Every response from `/api/audio/speech` carries:
