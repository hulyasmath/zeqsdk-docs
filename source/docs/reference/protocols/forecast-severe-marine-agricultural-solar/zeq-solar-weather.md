---
sidebar_position: 5
title: "ZeqSolarWeather"
description: "Space weather monitoring. Solar flare probability, geomagnetic storm index (Kp), aurora forecast, satellite drag estimation with HulyaPulse solar wind correlation."
---
...
Reach for **ZeqSolarWeather** when you need a forecast, severe, marine, agricultural, solar primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/weather/solar` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/weather/solar \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/weather/solar", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/weather/solar", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/weather/solar"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/weather/solar");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/weather/solar");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/weather/solar")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/weather/solar",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/weather/solar", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/weather/solar
...
Every response from `/api/weather/solar` carries:
