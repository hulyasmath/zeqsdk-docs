---
sidebar_position: 2
title: "ZeqSevereWeather"
description: "Severe weather detection and alerting. Tornado vortex signatures, hail probability, lightning prediction using R(t) convective instability indices (CAPE/CIN)."
---
...
Reach for **ZeqSevereWeather** when you need a forecast, severe, marine, agricultural, solar primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/weather/severe` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/weather/severe \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/weather/severe", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/weather/severe", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/weather/severe"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/weather/severe");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/weather/severe");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/weather/severe")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/weather/severe",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/weather/severe", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/weather/severe
...
Every response from `/api/weather/severe` carries:
