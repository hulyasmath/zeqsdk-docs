---
sidebar_position: 3
title: "ZeqMarineWeather"
description: "Marine weather and wave prediction. Significant wave height, swell period, current vectors with HulyaPulse tidal harmonics — critical for shipping and fishing."
---
...
Reach for **ZeqMarineWeather** when you need a forecast, severe, marine, agricultural, solar primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/weather/marine` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/weather/marine \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/weather/marine", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/weather/marine", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/weather/marine"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/weather/marine");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/weather/marine");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/weather/marine")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/weather/marine",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/weather/marine", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/weather/marine
...
Every response from `/api/weather/marine` carries:
