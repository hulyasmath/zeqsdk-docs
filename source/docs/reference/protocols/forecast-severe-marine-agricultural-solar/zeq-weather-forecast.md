---
sidebar_position: 1
title: "ZeqWeather Forecast"
description: "Numerical weather prediction with HulyaPulse temporal interpolation. GFS/ECMWF model fusion, R(t)-weighted ensemble averaging, hourly to 14-day forecasts."
---
...
Reach for **ZeqWeather Forecast** when you need a forecast, severe, marine, agricultural, solar primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/weather/forecast` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/weather/forecast \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/weather/forecast", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/weather/forecast", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/weather/forecast"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/weather/forecast");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/weather/forecast");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/weather/forecast")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/weather/forecast",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/weather/forecast", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/weather/forecast
...
Every response from `/api/weather/forecast` carries:
