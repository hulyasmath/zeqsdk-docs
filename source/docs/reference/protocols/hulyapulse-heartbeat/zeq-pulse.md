...
sidebar_position: 1
title: "ZeqPulse"
description: "Real-time HulyaPulse state — zeqond count, phase [0,1), R(t), field strength. The heartbeat of the framework."
---
...
Reach for **ZeqPulse** when you need a hulyapulse heartbeat primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/zeq/pulse` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/zeq/pulse \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/pulse", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/zeq/pulse", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/pulse"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/pulse");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/pulse");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/pulse")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/pulse",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/pulse", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/zeq/pulse
...
Every response from `/api/zeq/pulse` carries:
