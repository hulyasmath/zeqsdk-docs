---
sidebar_position: 3
title: "ZTB1 Timebase Bridge"
description: "Bidirectional Unix ↔ Zeqond time conversion. Lossless mapping between standard epoch time and HulyaPulse computational time."
---
...
Bidirectional Unix ↔ Zeqond time conversion. Lossless mapping between standard epoch time and HulyaPulse computational time. It belongs to the **HulyaPulse heartbeat** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/zeq/timebridge`, Zeq runs the **ZTB1 Timebase Bridge** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its hulyapulse heartbeat operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZTB1 Timebase Bridge** when you need a hulyapulse heartbeat primitive that:
...
If you only need a one-shot hulyapulse heartbeat answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/zeq/timebridge` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/zeq/timebridge \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/timebridge", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/zeq/timebridge", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/timebridge"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/timebridge");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/timebridge");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/timebridge")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/timebridge",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/timebridge", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/zeq/timebridge
...
Every response from `/api/zeq/timebridge` carries:
...
- All protocols in this family — see the [HulyaPulse heartbeat](/reference/protocols) category
