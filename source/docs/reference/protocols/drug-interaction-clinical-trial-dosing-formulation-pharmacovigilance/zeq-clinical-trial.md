---
sidebar_position: 2
title: "ZeqClinicalTrial"
description: "Clinical trial design and power analysis. Sample size calculation, randomization, interim analysis boundaries (O"
---
...
Clinical trial design and power analysis. Sample size calculation, randomization, interim analysis boundaries (O It belongs to the **Drug Interaction, Clinical Trial, Dosing, Formulation, Pharmacovigilance** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/pharma/clinical-trial`, Zeq runs the **ZeqClinicalTrial** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqClinicalTrial** when you need a drug interaction, clinical trial, dosing, formulation, pharmacovigilance primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/pharma/clinical-trial` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/pharma/clinical-trial \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/pharma/clinical-trial", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/pharma/clinical-trial", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/pharma/clinical-trial"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/pharma/clinical-trial");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/pharma/clinical-trial");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/pharma/clinical-trial")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/pharma/clinical-trial",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/pharma/clinical-trial", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/pharma/clinical-trial
...
Every response from `/api/pharma/clinical-trial` carries:
