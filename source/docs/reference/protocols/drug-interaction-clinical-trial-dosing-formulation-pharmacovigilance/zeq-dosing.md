...
sidebar_position: 3
title: "ZeqDosing"
description: "Individualized drug dosing. Population PK models (NONMEM-style), Bayesian dose adjustment, therapeutic drug monitoring with HulyaPulse sampling schedules."
---
...
Reach for **ZeqDosing** when you need a drug interaction, clinical trial, dosing, formulation, pharmacovigilance primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/pharma/dosing` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/pharma/dosing \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/pharma/dosing", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/pharma/dosing", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/pharma/dosing"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/pharma/dosing");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/pharma/dosing");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/pharma/dosing")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/pharma/dosing",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/pharma/dosing", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/pharma/dosing
...
Every response from `/api/pharma/dosing` carries:
