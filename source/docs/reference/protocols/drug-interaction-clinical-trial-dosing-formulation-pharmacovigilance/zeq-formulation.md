---
sidebar_position: 4
title: "ZeqFormulation"
description: "Drug formulation optimization. Dissolution modeling (Noyes-Whitney), excipient compatibility, stability prediction with R(t) degradation kinetics."
---
...
Reach for **ZeqFormulation** when you need a drug interaction, clinical trial, dosing, formulation, pharmacovigilance primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/pharma/formulation` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/pharma/formulation \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/pharma/formulation", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/pharma/formulation", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/pharma/formulation"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/pharma/formulation");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/pharma/formulation");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/pharma/formulation")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/pharma/formulation",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/pharma/formulation", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/pharma/formulation
...
Every response from `/api/pharma/formulation` carries:
