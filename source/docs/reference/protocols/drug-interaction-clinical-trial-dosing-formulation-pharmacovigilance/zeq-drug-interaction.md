---
sidebar_position: 1
title: "ZeqDrugInteraction"
description: "Drug-drug interaction checking. CYP450 metabolism prediction, DDI severity scoring, alternative drug suggestions with R(t)-modulated pharmacokinetic modeling."
---
...
| **Category** | Drug Interaction, Clinical Trial, Dosing, Formulation, Pharmacovigilance |
| **Version** | 1.287.0 |
...
Drug-drug interaction checking. CYP450 metabolism prediction, DDI severity scoring, alternative drug suggestions with R(t)-modulated pharmacokinetic modeling. It belongs to the **Drug Interaction, Clinical Trial, Dosing, Formulation, Pharmacovigilance** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/pharma/interaction`, Zeq runs the **ZeqDrugInteraction** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its drug interaction, clinical trial, dosing, formulation, pharmacovigilance operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqDrugInteraction** when you need a drug interaction, clinical trial, dosing, formulation, pharmacovigilance primitive that:
...
If you only need a one-shot drug interaction, clinical trial, dosing, formulation, pharmacovigilance answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/pharma/interaction` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/pharma/interaction \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/pharma/interaction", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/pharma/interaction", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/pharma/interaction"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/pharma/interaction");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/pharma/interaction");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/pharma/interaction")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/pharma/interaction",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/pharma/interaction", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/pharma/interaction
...
Every response from `/api/pharma/interaction` carries:
...
- All protocols in this family — see the [Drug Interaction, Clinical Trial, Dosing, Formulation, Pharmacovigilance](/reference/protocols) category
