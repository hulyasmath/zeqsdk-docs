---
sidebar_position: 5
title: "ZeqPharmacovigilance"
description: "Adverse event signal detection. Disproportionality analysis (PRR/ROR), Bayesian signal detection, causality assessment with Zeqond-timestamped reporting."
---
...
Reach for **ZeqPharmacovigilance** when you need a drug interaction, clinical trial, dosing, formulation, pharmacovigilance primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/pharma/vigilance` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/pharma/vigilance \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/pharma/vigilance", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/pharma/vigilance", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/pharma/vigilance"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/pharma/vigilance");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/pharma/vigilance");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/pharma/vigilance")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/pharma/vigilance",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/pharma/vigilance", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/pharma/vigilance
...
Every response from `/api/pharma/vigilance` carries:
