---
sidebar_position: 5
title: "ZeqDocumentForensics"
description: "Questioned document examination. Handwriting analysis, ink dating, paper fiber identification, alteration detection with R(t) spectral ink aging curves."
---
...
| **Category** | Digital, DNA, Fingerprint, Document |
| **Version** | 1.287.0 |
...
Questioned document examination. Handwriting analysis, ink dating, paper fiber identification, alteration detection with R(t) spectral ink aging curves. It belongs to the **Digital, DNA, Fingerprint, Document** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/forensics/document`, Zeq runs the **ZeqDocumentForensics** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its digital, dna, fingerprint, document operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqDocumentForensics** when you need a digital, dna, fingerprint, document primitive that:
...
If you only need a one-shot digital, dna, fingerprint, document answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/forensics/document` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/forensics/document \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/forensics/document", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/forensics/document", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/forensics/document"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/forensics/document");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/forensics/document");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/forensics/document")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/forensics/document",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/forensics/document", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/forensics/document
...
Every response from `/api/forensics/document` carries:
...
- All protocols in this family — see the [Digital, DNA, Fingerprint, Document](/reference/protocols) category
