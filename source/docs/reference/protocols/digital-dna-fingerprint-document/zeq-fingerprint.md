---
sidebar_position: 3
title: "ZeqFingerprint"
description: "Fingerprint analysis with R(t) ridge enhancement. Minutiae extraction, AFIS scoring, quality assessment, latent print processing."
---
...
Fingerprint analysis with R(t) ridge enhancement. Minutiae extraction, AFIS scoring, quality assessment, latent print processing. It belongs to the **Digital, DNA, Fingerprint, Document** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/forensics/fingerprint`, Zeq runs the **ZeqFingerprint** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqFingerprint** when you need a digital, dna, fingerprint, document primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/forensics/fingerprint` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/forensics/fingerprint \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/forensics/fingerprint", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/forensics/fingerprint", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/forensics/fingerprint"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/forensics/fingerprint");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/forensics/fingerprint");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/forensics/fingerprint")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/forensics/fingerprint",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/forensics/fingerprint", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/forensics/fingerprint
...
Every response from `/api/forensics/fingerprint` carries:
