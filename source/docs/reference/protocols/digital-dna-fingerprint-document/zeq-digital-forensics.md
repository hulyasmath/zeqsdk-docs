---
sidebar_position: 1
title: "ZeqDigitalForensics"
description: "Digital forensics evidence acquisition. Disk imaging with Zeqond hash chains, file carving, timeline reconstruction, chain-of-custody logging."
---
...
Digital forensics evidence acquisition. Disk imaging with Zeqond hash chains, file carving, timeline reconstruction, chain-of-custody logging. It belongs to the **Digital, DNA, Fingerprint, Document** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/forensics/digital`, Zeq runs the **ZeqDigitalForensics** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqDigitalForensics** when you need a digital, dna, fingerprint, document primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/forensics/digital` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/forensics/digital \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/forensics/digital", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/forensics/digital", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/forensics/digital"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/forensics/digital");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/forensics/digital");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/forensics/digital")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/forensics/digital",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/forensics/digital", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/forensics/digital
...
Every response from `/api/forensics/digital` carries:
