---
sidebar_position: 4
title: "ZeqDNS — Equation-Based Resolution"
description: "Domains are mathematical expressions, not strings. "
---
...
Reach for **ZeqDNS — Equation-Based Resolution** when you need a internet infrastructure replacement primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/network/dns/resolve` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/network/dns/resolve \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/network/dns/resolve", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/network/dns/resolve", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/network/dns/resolve"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/network/dns/resolve");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/network/dns/resolve");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/network/dns/resolve")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/network/dns/resolve",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/network/dns/resolve", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/network/dns/resolve
...
Every response from `/api/network/dns/resolve` carries:
