---
sidebar_position: 1
title: "ZeqSync — Packet Synchronization"
description: "Zeqond-grid packet synchronization. Every packet timestamped to the 0.777s grid — no clock drift, no NTP dependency. The replacement for network time protocols."
---
...
Reach for **ZeqSync — Packet Synchronization** when you need a internet infrastructure replacement primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/network/sync` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/network/sync \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/network/sync", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/network/sync", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/network/sync"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/network/sync");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/network/sync");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/network/sync")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/network/sync",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/network/sync", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/network/sync
...
Every response from `/api/network/sync` carries:
