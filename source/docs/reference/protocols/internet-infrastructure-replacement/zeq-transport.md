---
sidebar_position: 5
title: "ZeqTransport — Congestion Control"
description: "Congestion control using R(t) modulation instead of TCP window sizing. Send rate modulated by HulyaPulse — naturally oscillates to find optimal throughput without packet loss."
---
...
Reach for **ZeqTransport — Congestion Control** when you need a internet infrastructure replacement primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/network/transport/state` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/network/transport/state \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/network/transport/state", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/network/transport/state", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/network/transport/state"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/network/transport/state");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/network/transport/state");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/network/transport/state")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/network/transport/state",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/network/transport/state", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/network/transport/state
...
Every response from `/api/network/transport/state` carries:
