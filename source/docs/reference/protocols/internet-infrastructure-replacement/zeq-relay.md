---
sidebar_position: 6
title: "ZeqRelay — Zeqond-Stamped Relay"
description: "Message relay with Zeqond timestamps and ZeqProof integrity. Every relayed message carries proof of its origin time and path — tampering detectable by any node."
---
...
Reach for **ZeqRelay — Zeqond-Stamped Relay** when you need a internet infrastructure replacement primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/network/relay` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/network/relay \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/network/relay", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/network/relay", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/network/relay"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/network/relay");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/network/relay");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/network/relay")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/network/relay",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/network/relay", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/network/relay
...
Every response from `/api/network/relay` carries:
