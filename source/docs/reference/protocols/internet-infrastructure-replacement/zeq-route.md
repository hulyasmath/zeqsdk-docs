---
sidebar_position: 2
title: "ZeqRoute — Phase-Aware Routing"
description: "Shortest path weighted by HulyaPulse phase coherence. Nodes that are phase-aligned get priority — naturally selects the most synchronized route through the network."
---
...
Reach for **ZeqRoute — Phase-Aware Routing** when you need a internet infrastructure replacement primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/network/route` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/network/route \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/network/route", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/network/route", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/network/route"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/network/route");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/network/route");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/network/route")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/network/route",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/network/route", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/network/route
...
Every response from `/api/network/route` carries:
