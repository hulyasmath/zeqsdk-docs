---
sidebar_position: 7
title: "ZeqBandwidth — Pulse-Modulated Allocation"
description: "Bandwidth allocation modulated by HulyaPulse. High-priority traffic gets the peak of the R(t) cycle; background traffic uses the troughs. Fair by physics."
---
...
| **Category** | Internet infrastructure replacement |
| **Version** | 1.0 |
...
Bandwidth allocation modulated by HulyaPulse. High-priority traffic gets the peak of the R(t) cycle; background traffic uses the troughs. Fair by physics. It belongs to the **Internet infrastructure replacement** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/network/bandwidth/allocate`, Zeq runs the **ZeqBandwidth — Pulse-Modulated Allocation** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its internet infrastructure replacement operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqBandwidth — Pulse-Modulated Allocation** when you need a internet infrastructure replacement primitive that:
...
If you only need a one-shot internet infrastructure replacement answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/network/bandwidth/allocate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/network/bandwidth/allocate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/network/bandwidth/allocate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/network/bandwidth/allocate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/network/bandwidth/allocate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/network/bandwidth/allocate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/network/bandwidth/allocate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/network/bandwidth/allocate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/network/bandwidth/allocate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/network/bandwidth/allocate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/network/bandwidth/allocate
...
Every response from `/api/network/bandwidth/allocate` carries:
...
- All protocols in this family — see the [Internet infrastructure replacement](/reference/protocols) category
