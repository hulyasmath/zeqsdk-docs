---
sidebar_position: 2
title: "ZeqEcosystem — Population Dynamics"
description: "Population dynamics modeling (Lotka-Volterra + operator coupling). Predator-prey, competition, and mutualism simulated with the HULYAS ODE solver."
---
...
Population dynamics modeling (Lotka-Volterra + operator coupling). Predator-prey, competition, and mutualism simulated with the HULYAS ODE solver. It belongs to the **Protein, ecosystems, neural, genetics** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/bio/ecosystem/simulate`, Zeq runs the **ZeqEcosystem — Population Dynamics** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqEcosystem — Population Dynamics** when you need a protein, ecosystems, neural, genetics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/bio/ecosystem/simulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/bio/ecosystem/simulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/bio/ecosystem/simulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/bio/ecosystem/simulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/bio/ecosystem/simulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/bio/ecosystem/simulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/bio/ecosystem/simulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/bio/ecosystem/simulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/bio/ecosystem/simulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/bio/ecosystem/simulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/bio/ecosystem/simulate
...
Every response from `/api/bio/ecosystem/simulate` carries:
...
- All protocols in this family — see the [Protein, ecosystems, neural, genetics](/reference/protocols) category
