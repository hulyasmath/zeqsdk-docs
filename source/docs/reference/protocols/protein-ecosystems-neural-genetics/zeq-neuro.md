---
sidebar_position: 4
title: "ZeqNeuro — Neural Spike Analysis"
description: "Neural spike train analysis synchronized to HulyaPulse phase. Correlate neural firing patterns with the 1.287 Hz reference — detect phase-locking in brain activity."
---
...
Reach for **ZeqNeuro — Neural Spike Analysis** when you need a protein, ecosystems, neural, genetics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/bio/neuro/analyze` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/bio/neuro/analyze \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/bio/neuro/analyze", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/bio/neuro/analyze", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/bio/neuro/analyze"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/bio/neuro/analyze");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/bio/neuro/analyze");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/bio/neuro/analyze")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/bio/neuro/analyze",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/bio/neuro/analyze", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/bio/neuro/analyze
...
Every response from `/api/bio/neuro/analyze` carries:
