---
sidebar_position: 6
title: "ZeqEvolution — Evolutionary Dynamics"
description: "Evolutionary dynamics via replicator equations + mutation-selection balance. Fitness landscapes computed with operator coupling — evolution as a HULYAS field process."
---
...
Reach for **ZeqEvolution — Evolutionary Dynamics** when you need a protein, ecosystems, neural, genetics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/bio/evolution/simulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/bio/evolution/simulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/bio/evolution/simulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/bio/evolution/simulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/bio/evolution/simulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/bio/evolution/simulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/bio/evolution/simulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/bio/evolution/simulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/bio/evolution/simulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/bio/evolution/simulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/bio/evolution/simulate
...
Every response from `/api/bio/evolution/simulate` carries:
