---
sidebar_position: 1
title: "ZeqProtein — Folding Energy Landscape"
description: "Protein folding energy landscape via HULYAS functional equation E = P_φ · Z(M,R,δ,C,X). Maps amino acid sequences to energy states using operator coupling."
---
...
Reach for **ZeqProtein — Folding Energy Landscape** when you need a protein, ecosystems, neural, genetics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/bio/protein/fold` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/bio/protein/fold \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/bio/protein/fold", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/bio/protein/fold", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/bio/protein/fold"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/bio/protein/fold");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/bio/protein/fold");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/bio/protein/fold")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/bio/protein/fold",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/bio/protein/fold", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/bio/protein/fold
...
Every response from `/api/bio/protein/fold` carries:
