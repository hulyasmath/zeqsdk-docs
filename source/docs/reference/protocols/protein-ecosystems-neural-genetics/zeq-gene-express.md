---
sidebar_position: 5
title: "ZeqGene — Expression Analysis"
description: "Gene expression time-series analysis using ZeqShift projection. Map gene expression oscillations onto the HulyaPulse timescale to detect circadian and ultradian rhythms."
---
...
Reach for **ZeqGene — Expression Analysis** when you need a protein, ecosystems, neural, genetics primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/bio/gene/expression` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/bio/gene/expression \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/bio/gene/expression", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/bio/gene/expression", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/bio/gene/expression"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/bio/gene/expression");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/bio/gene/expression");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/bio/gene/expression")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/bio/gene/expression",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/bio/gene/expression", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/bio/gene/expression
...
Every response from `/api/bio/gene/expression` carries:
