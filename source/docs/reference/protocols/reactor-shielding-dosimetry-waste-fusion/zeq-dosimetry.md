...
sidebar_position: 3
title: "ZeqDosimetry"
description: "Personal dosimetry and ALARA optimization. Cumulative dose tracking, stay-time calculations, dose-rate contouring with Zeqond exposure logging."
---
...
Reach for **ZeqDosimetry** when you need a reactor, shielding, dosimetry, waste, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/nuclear/dosimetry` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nuclear/dosimetry \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nuclear/dosimetry", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nuclear/dosimetry", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nuclear/dosimetry"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nuclear/dosimetry");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nuclear/dosimetry");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nuclear/dosimetry")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nuclear/dosimetry",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nuclear/dosimetry", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nuclear/dosimetry
...
Every response from `/api/nuclear/dosimetry` carries:
