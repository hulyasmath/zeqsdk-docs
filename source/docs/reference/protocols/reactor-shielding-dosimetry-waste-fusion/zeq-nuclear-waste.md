---
sidebar_position: 4
title: "ZeqNuclearWaste"
description: "Nuclear waste management. Decay heat calculation, isotope inventory tracking, repository thermal modeling with R(t) long-term decay chain simulation."
---
...
Reach for **ZeqNuclearWaste** when you need a reactor, shielding, dosimetry, waste, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/nuclear/waste` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nuclear/waste \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nuclear/waste", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nuclear/waste", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nuclear/waste"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nuclear/waste");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nuclear/waste");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nuclear/waste")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nuclear/waste",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nuclear/waste", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nuclear/waste
...
Every response from `/api/nuclear/waste` carries:
