...
sidebar_position: 1
title: "ZeqReactor"
description: "Nuclear reactor core simulation. Neutron diffusion, criticality (k-eff), fuel burnup with R(t)-modulated control rod worth and xenon transient modeling."
---
...
| **Category** | Reactor, Shielding, Dosimetry, Waste, Fusion |
| **Version** | 1.287.0 |
...
Nuclear reactor core simulation. Neutron diffusion, criticality (k-eff), fuel burnup with R(t)-modulated control rod worth and xenon transient modeling. It belongs to the **Reactor, Shielding, Dosimetry, Waste, Fusion** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/nuclear/reactor`, Zeq runs the **ZeqReactor** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its reactor, shielding, dosimetry, waste, fusion operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqReactor** when you need a reactor, shielding, dosimetry, waste, fusion primitive that:
...
If you only need a one-shot reactor, shielding, dosimetry, waste, fusion answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/nuclear/reactor` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nuclear/reactor \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "coreGeometry": "<coreGeometry>",
  "controlRodPosition_pct": 1,
  "burnup_MWd_t": 1
}'
```
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nuclear/reactor", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nuclear/reactor", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nuclear/reactor"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nuclear/reactor");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nuclear/reactor");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nuclear/reactor")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nuclear/reactor",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nuclear/reactor", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nuclear/reactor
...
Every response from `/api/nuclear/reactor` carries:
...
- All protocols in this family — see the [Reactor, Shielding, Dosimetry, Waste, Fusion](/reference/protocols) category
