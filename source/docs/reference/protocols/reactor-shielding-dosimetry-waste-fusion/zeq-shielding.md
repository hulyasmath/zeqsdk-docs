...
sidebar_position: 2
title: "ZeqShielding"
description: "Radiation shielding design. Gamma attenuation (build-up factors), neutron moderation, dose rate mapping with R(t) source term decay curves."
---
...
Radiation shielding design. Gamma attenuation (build-up factors), neutron moderation, dose rate mapping with R(t) source term decay curves. It belongs to the **Reactor, Shielding, Dosimetry, Waste, Fusion** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/nuclear/shielding`, Zeq runs the **ZeqShielding** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqShielding** when you need a reactor, shielding, dosimetry, waste, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/nuclear/shielding` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/nuclear/shielding \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/nuclear/shielding", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/nuclear/shielding", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/nuclear/shielding"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/nuclear/shielding");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/nuclear/shielding");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/nuclear/shielding")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/nuclear/shielding",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/nuclear/shielding", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/nuclear/shielding
...
Every response from `/api/nuclear/shielding` carries:
