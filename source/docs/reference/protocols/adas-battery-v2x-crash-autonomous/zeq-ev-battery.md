...
sidebar_position: 2
title: "ZeqEVBattery"
description: "EV battery management with HulyaPulse cell balancing. State of charge/health estimation, thermal runaway prediction, optimal charging curves via R(t) electrochemistry."
---
...
Reach for **ZeqEVBattery** when you need a adas, battery, v2x, crash, autonomous primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/auto/ev-battery` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/auto/ev-battery \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auto/ev-battery", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/auto/ev-battery", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auto/ev-battery"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auto/ev-battery");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auto/ev-battery");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auto/ev-battery")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/auto/ev-battery",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auto/ev-battery", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/auto/ev-battery
...
Every response from `/api/auto/ev-battery` carries:
