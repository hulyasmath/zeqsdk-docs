...
sidebar_position: 1
title: "ZeqADAS"
description: "Advanced driver assistance — sensor fusion (camera/radar/LiDAR) on Zeqond frames, lane keeping, adaptive cruise control, collision avoidance with R(t) reaction timing."
---
...
Reach for **ZeqADAS** when you need a adas, battery, v2x, crash, autonomous primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/auto/adas` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/auto/adas \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auto/adas", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/auto/adas", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auto/adas"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auto/adas");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auto/adas");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auto/adas")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/auto/adas",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auto/adas", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/auto/adas
...
Every response from `/api/auto/adas` carries:
