...
sidebar_position: 5
title: "ZeqAutonomous"
description: "Full autonomous driving stack — perception/planning/control pipeline on Zeqond cycle. Behavior planning, trajectory optimization, safety envelope monitoring."
---
...
Reach for **ZeqAutonomous** when you need a adas, battery, v2x, crash, autonomous primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/auto/autonomous` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/auto/autonomous \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/auto/autonomous", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/auto/autonomous", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/auto/autonomous"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/auto/autonomous");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/auto/autonomous");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/auto/autonomous")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/auto/autonomous",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/auto/autonomous", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/auto/autonomous
...
Every response from `/api/auto/autonomous` carries:
