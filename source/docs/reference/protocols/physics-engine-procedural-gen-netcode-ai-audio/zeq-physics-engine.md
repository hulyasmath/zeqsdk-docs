---
sidebar_position: 1
title: "ZeqPhysicsEngine"
description: "Game physics engine with Zeqond-quantized timestep. Deterministic rigid body simulation at 0.777s tick rate, collision detection, constraint solving — same result on every machine."
---
...
| **Category** | Physics Engine, Procedural Gen, Netcode, AI, Audio |
| **Version** | 1.287.0 |
...
Game physics engine with Zeqond-quantized timestep. Deterministic rigid body simulation at 0.777s tick rate, collision detection, constraint solving — same result on every machine. It belongs to the **Physics Engine, Procedural Gen, Netcode, AI, Audio** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/gaming/physics`, Zeq runs the **ZeqPhysicsEngine** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its physics engine, procedural gen, netcode, ai, audio operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqPhysicsEngine** when you need a physics engine, procedural gen, netcode, ai, audio primitive that:
...
If you only need a one-shot physics engine, procedural gen, netcode, ai, audio answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/gaming/physics` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/gaming/physics \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/gaming/physics", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/gaming/physics", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/gaming/physics"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/gaming/physics");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/gaming/physics");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/gaming/physics")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/gaming/physics",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/gaming/physics", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/gaming/physics
...
Every response from `/api/gaming/physics` carries:
...
- All protocols in this family — see the [Physics Engine, Procedural Gen, Netcode, AI, Audio](/reference/protocols) category
