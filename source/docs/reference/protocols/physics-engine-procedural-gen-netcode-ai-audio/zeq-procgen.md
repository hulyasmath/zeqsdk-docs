...
sidebar_position: 2
title: "ZeqProcGen"
description: "Procedural content generation seeded by Zeqond state. Terrain, dungeons, galaxies, loot tables — all deterministic from HulyaPulse phase. Same seed, same world, always."
---
...
Reach for **ZeqProcGen** when you need a physics engine, procedural gen, netcode, ai, audio primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/gaming/procgen` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/gaming/procgen \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/gaming/procgen", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/gaming/procgen", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/gaming/procgen"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/gaming/procgen");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/gaming/procgen");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/gaming/procgen")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/gaming/procgen",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/gaming/procgen", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/gaming/procgen
...
Every response from `/api/gaming/procgen` carries:
