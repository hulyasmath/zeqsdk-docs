---
sidebar_position: 5
title: "ZeqSpatialAudio"
description: "3D spatial audio engine with HulyaPulse-synced HRTF. Zeqond-quantized reverb tails, Doppler simulation, occlusion via R(t)-weighted ray casting."
---
...
Reach for **ZeqSpatialAudio** when you need a physics engine, procedural gen, netcode, ai, audio primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/gaming/spatial-audio` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/gaming/spatial-audio \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/gaming/spatial-audio", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/gaming/spatial-audio", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/gaming/spatial-audio"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/gaming/spatial-audio");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/gaming/spatial-audio");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/gaming/spatial-audio")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/gaming/spatial-audio",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/gaming/spatial-audio", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/gaming/spatial-audio
...
Every response from `/api/gaming/spatial-audio` carries:
