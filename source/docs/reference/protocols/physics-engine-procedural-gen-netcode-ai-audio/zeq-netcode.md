...
sidebar_position: 3
title: "ZeqNetcode"
description: "Deterministic lockstep netcode on Zeqond ticks. 777ms authoritative frames, client-side prediction with R(t) rollback correction. Zero desync guaranteed."
---
...
Reach for **ZeqNetcode** when you need a physics engine, procedural gen, netcode, ai, audio primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/gaming/netcode` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/gaming/netcode \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/gaming/netcode", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/gaming/netcode", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/gaming/netcode"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/gaming/netcode");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/gaming/netcode");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/gaming/netcode")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/gaming/netcode",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/gaming/netcode", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/gaming/netcode
...
Every response from `/api/gaming/netcode` carries:
