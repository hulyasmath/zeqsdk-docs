...
sidebar_position: 4
title: "ZeqGameAI"
description: "NPC behavior trees and pathfinding with HulyaPulse decision cadence. A* on Zeqond grid, GOAP planning, flocking/steering with R(t) randomness injection for organic feel."
---
...
Reach for **ZeqGameAI** when you need a physics engine, procedural gen, netcode, ai, audio primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/gaming/ai` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/gaming/ai \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/gaming/ai", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/gaming/ai", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/gaming/ai"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/gaming/ai");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/gaming/ai");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/gaming/ai")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/gaming/ai",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/gaming/ai", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/gaming/ai
...
Every response from `/api/gaming/ai` carries:
