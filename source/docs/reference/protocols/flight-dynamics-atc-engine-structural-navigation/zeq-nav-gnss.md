...
sidebar_position: 5
title: "ZeqGNSS"
description: "GNSS positioning with HulyaPulse integrity monitoring. RAIM fault detection, multi-constellation (GPS/Galileo/GLONASS/BeiDou), ionospheric correction."
---
...
Reach for **ZeqGNSS** when you need a flight dynamics, atc, engine, structural, navigation primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/aviation/gnss` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/aviation/gnss \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/aviation/gnss", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/aviation/gnss", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/aviation/gnss"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/aviation/gnss");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/aviation/gnss");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/aviation/gnss")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/aviation/gnss",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/aviation/gnss", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/aviation/gnss
...
Every response from `/api/aviation/gnss` carries:
