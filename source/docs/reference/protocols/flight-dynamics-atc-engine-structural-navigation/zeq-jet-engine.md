...
sidebar_position: 3
title: "ZeqJetEngine"
description: "Gas turbine performance modeling. Brayton cycle with R(t) compressor map interpolation, turbine inlet temperature, SFC optimization, creep life prediction."
---
...
Reach for **ZeqJetEngine** when you need a flight dynamics, atc, engine, structural, navigation primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/aviation/jet-engine` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/aviation/jet-engine \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/aviation/jet-engine", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/aviation/jet-engine", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/aviation/jet-engine"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/aviation/jet-engine");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/aviation/jet-engine");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/aviation/jet-engine")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/aviation/jet-engine",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/aviation/jet-engine", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/aviation/jet-engine
...
Every response from `/api/aviation/jet-engine` carries:
