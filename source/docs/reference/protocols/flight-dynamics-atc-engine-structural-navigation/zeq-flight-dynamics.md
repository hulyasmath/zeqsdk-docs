---
sidebar_position: 1
title: "ZeqFlightDynamics"
description: "6-DOF flight dynamics with HulyaPulse integration. Lift/drag/thrust/weight balance, stability derivatives, control surface response at Zeqond timestep."
---
...
| **Category** | Flight Dynamics, ATC, Engine, Structural, Navigation |
| **Version** | 1.287.0 |
...
6-DOF flight dynamics with HulyaPulse integration. Lift/drag/thrust/weight balance, stability derivatives, control surface response at Zeqond timestep. It belongs to the **Flight Dynamics, ATC, Engine, Structural, Navigation** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/aviation/flight-dynamics`, Zeq runs the **ZeqFlightDynamics** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its flight dynamics, atc, engine, structural, navigation operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqFlightDynamics** when you need a flight dynamics, atc, engine, structural, navigation primitive that:
...
If you only need a one-shot flight dynamics, atc, engine, structural, navigation answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/aviation/flight-dynamics` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/aviation/flight-dynamics \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/aviation/flight-dynamics", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/aviation/flight-dynamics", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/aviation/flight-dynamics"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/aviation/flight-dynamics");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/aviation/flight-dynamics");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/aviation/flight-dynamics")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/aviation/flight-dynamics",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/aviation/flight-dynamics", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/aviation/flight-dynamics
...
Every response from `/api/aviation/flight-dynamics` carries:
...
- All protocols in this family — see the [Flight Dynamics, ATC, Engine, Structural, Navigation](/reference/protocols) category
