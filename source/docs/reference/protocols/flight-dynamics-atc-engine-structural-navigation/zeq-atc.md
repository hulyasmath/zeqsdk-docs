...
sidebar_position: 2
title: "ZeqATC"
description: "Air traffic control conflict detection and resolution. Zeqond-timed separation analysis, 4D trajectory prediction, holding pattern optimization."
---
...
Reach for **ZeqATC** when you need a flight dynamics, atc, engine, structural, navigation primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/aviation/atc` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/aviation/atc \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/aviation/atc", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/aviation/atc", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/aviation/atc"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/aviation/atc");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/aviation/atc");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/aviation/atc")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/aviation/atc",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/aviation/atc", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/aviation/atc
...
Every response from `/api/aviation/atc` carries:
