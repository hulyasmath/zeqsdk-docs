---
sidebar_position: 4
title: "ZeqAirframeFatigue"
description: "Airframe structural fatigue analysis. Miner"
---
...
Reach for **ZeqAirframeFatigue** when you need a flight dynamics, atc, engine, structural, navigation primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/aviation/fatigue` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/aviation/fatigue \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/aviation/fatigue", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/aviation/fatigue", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/aviation/fatigue"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/aviation/fatigue");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/aviation/fatigue");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/aviation/fatigue")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/aviation/fatigue",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/aviation/fatigue", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/aviation/fatigue
...
Every response from `/api/aviation/fatigue` carries:
