...
sidebar_position: 1
title: "ZeqChipDesign"
description: "IC timing analysis with HulyaPulse clock tree. Setup/hold verification at 0.777s boundaries, gate-level simulation, power estimation with R(t) leakage modeling."
---
...
Reach for **ZeqChipDesign** when you need a chip design, fpga, thermal, power, testing primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/hardware/chip` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hardware/chip \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hardware/chip", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hardware/chip", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hardware/chip"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hardware/chip");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hardware/chip");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hardware/chip")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hardware/chip",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hardware/chip", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hardware/chip
...
Every response from `/api/hardware/chip` carries:
