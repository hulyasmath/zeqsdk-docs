...
sidebar_position: 5
title: "ZeqChipTest"
description: "Automated test pattern generation (ATPG) with Zeqond scan chains. Fault coverage analysis, BIST integration, yield prediction using R(t) defect density modeling."
---
...
Reach for **ZeqChipTest** when you need a chip design, fpga, thermal, power, testing primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/hardware/test` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hardware/test \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "netlist": "<netlist>",
  "faultModel": "<faultModel>",
  "targetCoverage_pct": 1
}'
```
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hardware/test", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hardware/test", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hardware/test"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hardware/test");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hardware/test");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hardware/test")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hardware/test",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hardware/test", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hardware/test
...
Every response from `/api/hardware/test` carries:
