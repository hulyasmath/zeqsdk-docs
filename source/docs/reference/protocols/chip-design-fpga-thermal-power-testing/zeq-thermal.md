...
sidebar_position: 3
title: "ZeqThermal"
description: "Chip thermal simulation with R(t)-modulated heat dissipation. Junction temperature prediction, hotspot identification, cooling solution sizing."
---
...
Chip thermal simulation with R(t)-modulated heat dissipation. Junction temperature prediction, hotspot identification, cooling solution sizing. It belongs to the **Chip Design, FPGA, Thermal, Power, Testing** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/hardware/thermal`, Zeq runs the **ZeqThermal** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqThermal** when you need a chip design, fpga, thermal, power, testing primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/hardware/thermal` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hardware/thermal \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hardware/thermal", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hardware/thermal", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hardware/thermal"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hardware/thermal");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hardware/thermal");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hardware/thermal")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hardware/thermal",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hardware/thermal", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hardware/thermal
...
Every response from `/api/hardware/thermal` carries:
