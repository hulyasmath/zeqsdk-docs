---
sidebar_position: 4
title: "ZeqPowerIntegrity"
description: "PDN analysis with HulyaPulse noise characterization. IR drop, AC impedance, decoupling capacitor placement optimization synced to Zeqond switching transients."
---
...
| **Category** | Chip Design, FPGA, Thermal, Power, Testing |
| **Version** | 1.287.0 |
...
PDN analysis with HulyaPulse noise characterization. IR drop, AC impedance, decoupling capacitor placement optimization synced to Zeqond switching transients. It belongs to the **Chip Design, FPGA, Thermal, Power, Testing** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/hardware/power-integrity`, Zeq runs the **ZeqPowerIntegrity** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its chip design, fpga, thermal, power, testing operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqPowerIntegrity** when you need a chip design, fpga, thermal, power, testing primitive that:
...
If you only need a one-shot chip design, fpga, thermal, power, testing answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/hardware/power-integrity` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/hardware/power-integrity \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/hardware/power-integrity", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/hardware/power-integrity", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/hardware/power-integrity"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/hardware/power-integrity");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/hardware/power-integrity");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/hardware/power-integrity")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/hardware/power-integrity",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/hardware/power-integrity", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/hardware/power-integrity
...
Every response from `/api/hardware/power-integrity` carries:
...
- All protocols in this family — see the [Chip Design, FPGA, Thermal, Power, Testing](/reference/protocols) category
