---
sidebar_position: 3
title: "ZeqFiberOptics"
description: "Fiber optic link design. Attenuation, dispersion, nonlinear effects (SPM/XPM/FWM) with HulyaPulse chromatic dispersion compensation."
---
...
Fiber optic link design. Attenuation, dispersion, nonlinear effects (SPM/XPM/FWM) with HulyaPulse chromatic dispersion compensation. It belongs to the **Lens, Laser, Fiber, Spectroscopy, Holography** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/optics/fiber`, Zeq runs the **ZeqFiberOptics** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqFiberOptics** when you need a lens, laser, fiber, spectroscopy, holography primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/optics/fiber` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/optics/fiber \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/optics/fiber", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/optics/fiber", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/optics/fiber"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/optics/fiber");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/optics/fiber");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/optics/fiber")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/optics/fiber",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/optics/fiber", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/optics/fiber
...
Every response from `/api/optics/fiber` carries:
