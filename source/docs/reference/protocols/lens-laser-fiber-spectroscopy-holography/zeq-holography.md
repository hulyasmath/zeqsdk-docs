...
sidebar_position: 5
title: "ZeqHolography"
description: "Digital holography reconstruction. Fresnel propagation, phase unwrapping, 3D surface profiling with R(t)-compensated phase drift correction."
---
...
Digital holography reconstruction. Fresnel propagation, phase unwrapping, 3D surface profiling with R(t)-compensated phase drift correction. It belongs to the **Lens, Laser, Fiber, Spectroscopy, Holography** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/optics/holography`, Zeq runs the **ZeqHolography** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqHolography** when you need a lens, laser, fiber, spectroscopy, holography primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/optics/holography` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/optics/holography \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/optics/holography", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/optics/holography", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/optics/holography"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/optics/holography");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/optics/holography");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/optics/holography")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/optics/holography",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/optics/holography", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/optics/holography
...
Every response from `/api/optics/holography` carries:
