...
sidebar_position: 1
title: "ZeqLensDesign"
description: "Optical lens system design with R(t) aberration analysis. Ray tracing, Seidel aberrations, MTF curves, tolerance analysis — HulyaPulse-weighted merit function optimization."
---
...
Reach for **ZeqLensDesign** when you need a lens, laser, fiber, spectroscopy, holography primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/optics/lens` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/optics/lens \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/optics/lens", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/optics/lens", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/optics/lens"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/optics/lens");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/optics/lens");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/optics/lens")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/optics/lens",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/optics/lens", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/optics/lens
...
Every response from `/api/optics/lens` carries:
