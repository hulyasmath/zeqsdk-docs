---
sidebar_position: 4
title: "ZeqSpectroscopy"
description: "Spectral analysis with HulyaPulse modulation. UV-Vis, IR, Raman, fluorescence peak fitting, compound identification, concentration quantification via Beer-Lambert."
---
...
| **Category** | Lens, Laser, Fiber, Spectroscopy, Holography |
| **Version** | 1.287.0 |
...
Spectral analysis with HulyaPulse modulation. UV-Vis, IR, Raman, fluorescence peak fitting, compound identification, concentration quantification via Beer-Lambert. It belongs to the **Lens, Laser, Fiber, Spectroscopy, Holography** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/optics/spectroscopy`, Zeq runs the **ZeqSpectroscopy** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its lens, laser, fiber, spectroscopy, holography operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqSpectroscopy** when you need a lens, laser, fiber, spectroscopy, holography primitive that:
...
If you only need a one-shot lens, laser, fiber, spectroscopy, holography answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/optics/spectroscopy` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/optics/spectroscopy \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/optics/spectroscopy", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/optics/spectroscopy", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/optics/spectroscopy"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/optics/spectroscopy");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/optics/spectroscopy");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/optics/spectroscopy")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/optics/spectroscopy",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/optics/spectroscopy", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/optics/spectroscopy
...
Every response from `/api/optics/spectroscopy` carries:
...
- All protocols in this family — see the [Lens, Laser, Fiber, Spectroscopy, Holography](/reference/protocols) category
