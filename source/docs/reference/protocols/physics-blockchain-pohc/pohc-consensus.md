---
sidebar_position: 2
title: "PoHC — Consensus Check"
description: "Check consensus state for the current HulyaPulse cycle. Returns convergence ratio, validator count, and whether a block was finalized."
---
...
Check consensus state for the current HulyaPulse cycle. Returns convergence ratio, validator count, and whether a block was finalized. It belongs to the **Physics Blockchain, PoHC** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/chain/pohc/consensus`, Zeq runs the **PoHC — Consensus Check** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **PoHC — Consensus Check** when you need a physics blockchain, pohc primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/chain/pohc/consensus` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/chain/pohc/consensus \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/chain/pohc/consensus", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/chain/pohc/consensus", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/chain/pohc/consensus"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/chain/pohc/consensus");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/chain/pohc/consensus");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/chain/pohc/consensus")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/chain/pohc/consensus",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/chain/pohc/consensus", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/chain/pohc/consensus
...
Every response from `/api/chain/pohc/consensus` carries:
