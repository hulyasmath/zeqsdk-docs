---
sidebar_position: 3
title: "Physics Chain — Submit Transaction"
description: "Submit a transaction to the Physics Blockchain. Transactions are equation-signed (Zeq Auth) and carry optional physics computation data."
---
...
Submit a transaction to the Physics Blockchain. Transactions are equation-signed (Zeq Auth) and carry optional physics computation data. It belongs to the **Physics Blockchain, PoHC** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/chain/tx/submit`, Zeq runs the **Physics Chain — Submit Transaction** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **Physics Chain — Submit Transaction** when you need a physics blockchain, pohc primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/chain/tx/submit` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/chain/tx/submit \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/chain/tx/submit", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/chain/tx/submit", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/chain/tx/submit"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/chain/tx/submit");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/chain/tx/submit");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/chain/tx/submit")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/chain/tx/submit",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/chain/tx/submit", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/chain/tx/submit
...
Every response from `/api/chain/tx/submit` carries:
