---
sidebar_position: 6
title: "Physics Chain — Explorer"
description: "Search and browse the Physics Blockchain. Query transactions by ZID, explore blocks by range, view validator statistics."
---
...
Search and browse the Physics Blockchain. Query transactions by ZID, explore blocks by range, view validator statistics. It belongs to the **Physics Blockchain, PoHC** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/chain/explore`, Zeq runs the **Physics Chain — Explorer** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **Physics Chain — Explorer** when you need a physics blockchain, pohc primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/chain/explore` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/chain/explore \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/chain/explore", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/chain/explore", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/chain/explore"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/chain/explore");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/chain/explore");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/chain/explore")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/chain/explore",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/chain/explore", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/chain/explore
...
Every response from `/api/chain/explore` carries:
