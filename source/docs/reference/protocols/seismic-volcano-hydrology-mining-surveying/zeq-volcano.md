...
sidebar_position: 2
title: "ZeqVolcano"
description: "Volcanic activity monitoring. Seismic tremor analysis, SO2 flux, deformation (InSAR), eruption probability with HulyaPulse harmonic tremor detection."
---
...
Reach for **ZeqVolcano** when you need a seismic, volcano, hydrology, mining, surveying primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/geo/volcano` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/geo/volcano \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/geo/volcano", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/geo/volcano", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/geo/volcano"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/geo/volcano");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/geo/volcano");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/geo/volcano")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/geo/volcano",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/geo/volcano", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/geo/volcano
...
Every response from `/api/geo/volcano` carries:
