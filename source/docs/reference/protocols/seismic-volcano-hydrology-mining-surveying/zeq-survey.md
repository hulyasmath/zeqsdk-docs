...
sidebar_position: 5
title: "ZeqSurvey"
description: "Geodetic surveying with HulyaPulse coordinate transformations. Datum conversions, least-squares adjustment, geoid undulation — Zeqond-timestamped for temporal surveys."
---
...
Reach for **ZeqSurvey** when you need a seismic, volcano, hydrology, mining, surveying primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/geo/survey` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/geo/survey \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/geo/survey", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/geo/survey", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/geo/survey"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/geo/survey");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/geo/survey");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/geo/survey")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/geo/survey",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/geo/survey", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/geo/survey
...
Every response from `/api/geo/survey` carries:
