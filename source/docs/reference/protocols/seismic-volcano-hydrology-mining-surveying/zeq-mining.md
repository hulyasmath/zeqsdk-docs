...
sidebar_position: 4
title: "ZeqMining"
description: "Mining engineering. Ore grade estimation (kriging), blast pattern design, slope stability, ventilation network analysis with Zeqond-timed sensor integration."
---
...
Reach for **ZeqMining** when you need a seismic, volcano, hydrology, mining, surveying primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/geo/mining` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/geo/mining \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/geo/mining", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/geo/mining", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/geo/mining"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/geo/mining");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/geo/mining");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/geo/mining")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/geo/mining",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/geo/mining", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/geo/mining
...
Every response from `/api/geo/mining` carries:
