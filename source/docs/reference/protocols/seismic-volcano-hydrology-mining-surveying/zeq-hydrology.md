...
sidebar_position: 3
title: "ZeqHydrology"
description: "Watershed hydrology modeling. Rainfall-runoff (SCS-CN), flood frequency analysis, groundwater flow (Darcy) with R(t)-modulated infiltration rates."
---
...
Reach for **ZeqHydrology** when you need a seismic, volcano, hydrology, mining, surveying primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/geo/hydrology` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/geo/hydrology \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/geo/hydrology", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/geo/hydrology", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/geo/hydrology"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/geo/hydrology");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/geo/hydrology");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/geo/hydrology")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/geo/hydrology",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/geo/hydrology", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/geo/hydrology
...
Every response from `/api/geo/hydrology` carries:
