---
sidebar_position: 3
title: "ZeqAuditFin — Financial Audit Trail"
description: "Financial transaction audit with ZeqProof chains. Every transaction cryptographically linked to the exact Zeqond — immutable, verifiable, tamper-evident."
---
...
Financial transaction audit with ZeqProof chains. Every transaction cryptographically linked to the exact Zeqond — immutable, verifiable, tamper-evident. It belongs to the **Risk, pricing, audit** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/finance/audit/record`, Zeq runs the **ZeqAuditFin — Financial Audit Trail** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqAuditFin — Financial Audit Trail** when you need a risk, pricing, audit primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/finance/audit/record` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/finance/audit/record \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/finance/audit/record", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/finance/audit/record", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/finance/audit/record"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/finance/audit/record");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/finance/audit/record");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/finance/audit/record")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/finance/audit/record",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/finance/audit/record", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/finance/audit/record
...
Every response from `/api/finance/audit/record` carries:
