---
sidebar_position: 4
title: "ZeqDerivatives — Options Pricing"
description: "Options pricing using Black-Scholes and binomial models computed via the HULYAS solver. Greeks (delta, gamma, theta, vega, rho) at each Zeqond."
---
...
Options pricing using Black-Scholes and binomial models computed via the HULYAS solver. Greeks (delta, gamma, theta, vega, rho) at each Zeqond. It belongs to the **Risk, pricing, audit** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/finance/derivatives/price`, Zeq runs the **ZeqDerivatives — Options Pricing** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqDerivatives — Options Pricing** when you need a risk, pricing, audit primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/finance/derivatives/price` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/finance/derivatives/price \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/finance/derivatives/price", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/finance/derivatives/price", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/finance/derivatives/price"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/finance/derivatives/price");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/finance/derivatives/price");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/finance/derivatives/price")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/finance/derivatives/price",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/finance/derivatives/price", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/finance/derivatives/price
...
Every response from `/api/finance/derivatives/price` carries:
...
- All protocols in this family — see the [Risk, pricing, audit](/reference/protocols) category
