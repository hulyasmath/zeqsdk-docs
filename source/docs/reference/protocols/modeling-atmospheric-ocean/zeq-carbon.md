---
sidebar_position: 4
title: "ZeqCarbon — Carbon Cycle"
description: "Carbon cycle modeling — atmospheric CO2, ocean absorption, terrestrial sinks. Coupled ODE system tracking carbon flux between reservoirs."
---
...
Carbon cycle modeling — atmospheric CO2, ocean absorption, terrestrial sinks. Coupled ODE system tracking carbon flux between reservoirs. It belongs to the **Modeling, atmospheric, ocean** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/climate/carbon/model`, Zeq runs the **ZeqCarbon — Carbon Cycle** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqCarbon — Carbon Cycle** when you need a modeling, atmospheric, ocean primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/climate/carbon/model` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/climate/carbon/model \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "emissionsGtPerYear": 1,
  "projectionYears": 1
}'
```
...
```javascript
const res = await fetch("https://www.zeq.dev/api/climate/carbon/model", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/climate/carbon/model", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/climate/carbon/model"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/climate/carbon/model");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/climate/carbon/model");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/climate/carbon/model")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/climate/carbon/model",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/climate/carbon/model", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/climate/carbon/model
...
Every response from `/api/climate/carbon/model` carries:
