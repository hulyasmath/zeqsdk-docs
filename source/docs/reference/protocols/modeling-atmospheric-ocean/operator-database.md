---
sidebar_position: 7
title: "Operator Database Browser"
description: "Paginated, searchable operator database combining generated (1,340+) and canonical (1,576) operators."
---
...
Paginated, searchable operator database combining generated (1,340+) and canonical (1,576) operators. It belongs to the **Modeling, atmospheric, ocean** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/framework/registry`, Zeq runs the **Operator Database Browser** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **Operator Database Browser** when you need a modeling, atmospheric, ocean primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/framework/registry` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/framework/registry \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/framework/registry", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/framework/registry", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/framework/registry"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/framework/registry");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/framework/registry");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/framework/registry")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/framework/registry",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/framework/registry", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/framework/registry
...
Every response from `/api/framework/registry` carries:
