---
sidebar_position: 3
title: "ZeqOcean — Ocean Current Modeling"
description: "Ocean circulation modeling using fluid dynamics and geophysics operators. Thermohaline circulation, surface currents, and heat transport."
---
...
Ocean circulation modeling using fluid dynamics and geophysics operators. Thermohaline circulation, surface currents, and heat transport. It belongs to the **Modeling, atmospheric, ocean** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/climate/ocean/simulate`, Zeq runs the **ZeqOcean — Ocean Current Modeling** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqOcean — Ocean Current Modeling** when you need a modeling, atmospheric, ocean primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/climate/ocean/simulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/climate/ocean/simulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/climate/ocean/simulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/climate/ocean/simulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/climate/ocean/simulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/climate/ocean/simulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/climate/ocean/simulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/climate/ocean/simulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/climate/ocean/simulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/climate/ocean/simulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/climate/ocean/simulate
...
Every response from `/api/climate/ocean/simulate` carries:
