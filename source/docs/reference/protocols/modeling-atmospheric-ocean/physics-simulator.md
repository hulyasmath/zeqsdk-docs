...
description: "Real-time physics simulation synced to HulyaPulse. API provides pulse stream + computation; frontend renders."
---
...
Real-time physics simulation synced to HulyaPulse. API provides pulse stream + computation; frontend renders. It belongs to the **Modeling, atmospheric, ocean** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/zeq/pulse/stream`, Zeq runs the **3D Physics Simulator** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **3D Physics Simulator** when you need a modeling, atmospheric, ocean primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/zeq/pulse/stream` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/zeq/pulse/stream \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/zeq/pulse/stream", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/zeq/pulse/stream", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/zeq/pulse/stream"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/zeq/pulse/stream");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/zeq/pulse/stream");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/zeq/pulse/stream")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/zeq/pulse/stream",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/zeq/pulse/stream", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/zeq/pulse/stream
...
Every response from `/api/zeq/pulse/stream` carries:
