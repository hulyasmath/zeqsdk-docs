---
sidebar_position: 2
title: "ZeqAtmosphere — Atmospheric Physics"
description: "Atmospheric modeling — pressure, temperature, density profiles computed from first principles. Supports standard atmosphere, tropical, polar, and custom profiles."
---
...
| **Category** | Modeling, atmospheric, ocean |
| **Version** | 1.0 |
...
Atmospheric modeling — pressure, temperature, density profiles computed from first principles. Supports standard atmosphere, tropical, polar, and custom profiles. It belongs to the **Modeling, atmospheric, ocean** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/climate/atmosphere/profile`, Zeq runs the **ZeqAtmosphere — Atmospheric Physics** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its modeling, atmospheric, ocean operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqAtmosphere — Atmospheric Physics** when you need a modeling, atmospheric, ocean primitive that:
...
If you only need a one-shot modeling, atmospheric, ocean answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/climate/atmosphere/profile` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/climate/atmosphere/profile \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/climate/atmosphere/profile", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/climate/atmosphere/profile", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/climate/atmosphere/profile"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/climate/atmosphere/profile");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/climate/atmosphere/profile");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/climate/atmosphere/profile")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/climate/atmosphere/profile",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/climate/atmosphere/profile", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/climate/atmosphere/profile
...
Every response from `/api/climate/atmosphere/profile` carries:
...
- All protocols in this family — see the [Modeling, atmospheric, ocean](/reference/protocols) category
