...
description: "Interactive API playground. Demo key for testing compute, lattice, shift, and solve without authentication."
---
...
Interactive API playground. Demo key for testing compute, lattice, shift, and solve without authentication. It belongs to the **Modeling, atmospheric, ocean** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/playground/compute`, Zeq runs the **SDK Playground** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **SDK Playground** when you need a modeling, atmospheric, ocean primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/playground/compute` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/playground/compute \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/playground/compute", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/playground/compute", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/playground/compute"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/playground/compute");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/playground/compute");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/playground/compute")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/playground/compute",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/playground/compute", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/playground/compute
...
Every response from `/api/playground/compute` carries:
