...
description: "Core framework tuning — pulse frequency, Zeqond duration, alpha, precision target, max operators."
---
...
Reach for **Framework Parameters** when you need a operators, constants, equations primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/framework/params` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/framework/params \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/framework/params", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/framework/params", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/framework/params"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/framework/params");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/framework/params");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/framework/params")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/framework/params",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/framework/params", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/framework/params
...
Every response from `/api/framework/params` carries:
