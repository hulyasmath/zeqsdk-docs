---
sidebar_position: 6
title: "Single Constant Lookup"
description: "Look up a single physical constant by key."
---
...
Reach for **Single Constant Lookup** when you need a operators, constants, equations primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/framework/constants/:key` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/framework/constants/:key \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/framework/constants/:key", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/framework/constants/:key", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/framework/constants/:key"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/framework/constants/:key");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/framework/constants/:key");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/framework/constants/:key")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/framework/constants/:key",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/framework/constants/:key", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/framework/constants/:key
...
Every response from `/api/framework/constants/:key` carries:
