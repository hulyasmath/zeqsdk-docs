---
sidebar_position: 4
title: "Canonical Operator Lookup"
description: "Single operator with full LaTeX, founder, tier, and source attribution."
---
...
Reach for **Canonical Operator Lookup** when you need a operators, constants, equations primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/framework/operator/:id` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/framework/operator/:id \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/framework/operator/:id", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/framework/operator/:id", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/framework/operator/:id"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/framework/operator/:id");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/framework/operator/:id");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/framework/operator/:id")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/framework/operator/:id",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/framework/operator/:id", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/framework/operator/:id
...
Every response from `/api/framework/operator/:id` carries:
