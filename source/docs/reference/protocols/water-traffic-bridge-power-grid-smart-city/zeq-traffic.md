...
sidebar_position: 2
title: "ZeqTraffic"
description: "Traffic signal optimization and flow modeling. Zeqond-timed green wave synchronization, congestion prediction, accident probability scoring using R(t) vehicle density."
---
...
Reach for **ZeqTraffic** when you need a water, traffic, bridge, power grid, smart city primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/infra/traffic` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/infra/traffic \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/infra/traffic", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/infra/traffic", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/infra/traffic"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/infra/traffic");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/infra/traffic");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/infra/traffic")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/infra/traffic",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/infra/traffic", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/infra/traffic
...
Every response from `/api/infra/traffic` carries:
