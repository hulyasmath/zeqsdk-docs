...
sidebar_position: 4
title: "ZeqSmartGrid"
description: "Smart city power grid management. Zeqond-synchronized load balancing across distributed generation, demand response, and storage with R(t) frequency regulation."
---
...
| **Category** | Water, Traffic, Bridge, Power Grid, Smart City |
| **Version** | 1.287.0 |
...
Smart city power grid management. Zeqond-synchronized load balancing across distributed generation, demand response, and storage with R(t) frequency regulation. It belongs to the **Water, Traffic, Bridge, Power Grid, Smart City** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/infra/smart-grid`, Zeq runs the **ZeqSmartGrid** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its water, traffic, bridge, power grid, smart city operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqSmartGrid** when you need a water, traffic, bridge, power grid, smart city primitive that:
...
If you only need a one-shot water, traffic, bridge, power grid, smart city answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/infra/smart-grid` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/infra/smart-grid \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "gridTopology": "<gridTopology>",
  "storageCapacity_kWh": 1
}'
```
...
```javascript
const res = await fetch("https://www.zeq.dev/api/infra/smart-grid", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/infra/smart-grid", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/infra/smart-grid"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/infra/smart-grid");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/infra/smart-grid");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/infra/smart-grid")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/infra/smart-grid",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/infra/smart-grid", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/infra/smart-grid
...
Every response from `/api/infra/smart-grid` carries:
...
- All protocols in this family — see the [Water, Traffic, Bridge, Power Grid, Smart City](/reference/protocols) category
