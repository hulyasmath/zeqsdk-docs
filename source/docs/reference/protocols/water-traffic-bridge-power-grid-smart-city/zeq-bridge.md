...
sidebar_position: 3
title: "ZeqBridge"
description: "Structural health monitoring for bridges. HulyaPulse resonance analysis on accelerometer data, fatigue cycle counting, load distribution with R(t) stress amplification."
---
...
Reach for **ZeqBridge** when you need a water, traffic, bridge, power grid, smart city primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/infra/bridge` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/infra/bridge \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/infra/bridge", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/infra/bridge", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/infra/bridge"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/infra/bridge");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/infra/bridge");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/infra/bridge")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/infra/bridge",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/infra/bridge", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/infra/bridge
...
Every response from `/api/infra/bridge` carries:
