...
sidebar_position: 5
title: "ZeqSmartCity"
description: "Integrated smart city sensor fusion. Combines traffic, air quality, noise, energy, waste into one HulyaPulse-synchronized dashboard with anomaly detection."
---
...
Reach for **ZeqSmartCity** when you need a water, traffic, bridge, power grid, smart city primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/infra/smart-city` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/infra/smart-city \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/infra/smart-city", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/infra/smart-city", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/infra/smart-city"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/infra/smart-city");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/infra/smart-city");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/infra/smart-city")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/infra/smart-city",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/infra/smart-city", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/infra/smart-city
...
Every response from `/api/infra/smart-city` carries:
