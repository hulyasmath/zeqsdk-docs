...
sidebar_position: 1
title: "ZeqWater"
description: "Water treatment and distribution network optimization. HulyaPulse-modulated flow control, contaminant detection via R(t) spectral analysis, pressure zone balancing."
---
...
Reach for **ZeqWater** when you need a water, traffic, bridge, power grid, smart city primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/infra/water` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/infra/water \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/infra/water", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/infra/water", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/infra/water"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/infra/water");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/infra/water");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/infra/water")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/infra/water",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/infra/water", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/infra/water
...
Every response from `/api/infra/water` carries:
