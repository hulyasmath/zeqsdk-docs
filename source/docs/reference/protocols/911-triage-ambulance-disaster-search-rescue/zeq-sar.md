---
sidebar_position: 5
title: "ZeqSearch & Rescue"
description: "Search and rescue probability mapping. Bayesian search theory with HulyaPulse-timed sweep patterns, terrain analysis, survival probability decay curves."
---
...
Reach for **ZeqSearch & Rescue** when you need a 911, triage, ambulance, disaster, search & rescue primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/emergency/sar` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/emergency/sar \
  -H "Authorization: Bearer $ZEQ_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "terrain": "<terrain>",
  "timeSinceLost_hours": 1
}'
```
...
```javascript
const res = await fetch("https://www.zeq.dev/api/emergency/sar", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/emergency/sar", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/emergency/sar"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/emergency/sar");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/emergency/sar");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/emergency/sar")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/emergency/sar",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/emergency/sar", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/emergency/sar
...
Every response from `/api/emergency/sar` carries:
