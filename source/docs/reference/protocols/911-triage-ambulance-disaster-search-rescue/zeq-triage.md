...
sidebar_position: 2
title: "ZeqTriage"
description: "Mass casualty triage using START/JumpSTART with HulyaPulse scoring. Automated color classification (black/red/yellow/green) with Zeqond timestamps per assessment."
---
...
Reach for **ZeqTriage** when you need a 911, triage, ambulance, disaster, search & rescue primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/emergency/triage` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/emergency/triage \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/emergency/triage", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/emergency/triage", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/emergency/triage"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/emergency/triage");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/emergency/triage");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/emergency/triage")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/emergency/triage",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/emergency/triage", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/emergency/triage
...
Every response from `/api/emergency/triage` carries:
