...
sidebar_position: 3
title: "ZeqAmbulance"
description: "Ambulance routing with HulyaPulse traffic-aware optimization. Dynamic rerouting every Zeqond cycle, hospital capacity checking, golden hour countdown."
---
...
Reach for **ZeqAmbulance** when you need a 911, triage, ambulance, disaster, search & rescue primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/emergency/ambulance` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/emergency/ambulance \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/emergency/ambulance", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/emergency/ambulance", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/emergency/ambulance"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/emergency/ambulance");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/emergency/ambulance");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/emergency/ambulance")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/emergency/ambulance",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/emergency/ambulance", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/emergency/ambulance
...
Every response from `/api/emergency/ambulance` carries:
