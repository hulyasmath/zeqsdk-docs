...
sidebar_position: 4
title: "ZeqDisaster"
description: "Disaster response coordination. Multi-agency resource allocation, evacuation zone modeling, supply chain logistics with Zeqond-synchronized command timeline."
---
...
| **Category** | 911, Triage, Ambulance, Disaster, Search & Rescue |
| **Version** | 1.287.0 |
...
Disaster response coordination. Multi-agency resource allocation, evacuation zone modeling, supply chain logistics with Zeqond-synchronized command timeline. It belongs to the **911, Triage, Ambulance, Disaster, Search & Rescue** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/emergency/disaster`, Zeq runs the **ZeqDisaster** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its 911, triage, ambulance, disaster, search & rescue operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqDisaster** when you need a 911, triage, ambulance, disaster, search & rescue primitive that:
...
If you only need a one-shot 911, triage, ambulance, disaster, search & rescue answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/emergency/disaster` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/emergency/disaster \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/emergency/disaster", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/emergency/disaster", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/emergency/disaster"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/emergency/disaster");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/emergency/disaster");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/emergency/disaster")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/emergency/disaster",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/emergency/disaster", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/emergency/disaster
...
Every response from `/api/emergency/disaster` carries:
...
- All protocols in this family — see the [911, Triage, Ambulance, Disaster, Search & Rescue](/reference/protocols) category
