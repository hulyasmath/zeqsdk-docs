...
sidebar_position: 1
title: "Zeq911"
description: "Emergency dispatch optimization. Zeqond-stamped incident logging, nearest-resource allocation using R(t)-weighted distance metrics, priority queuing by severity."
---
...
Reach for **Zeq911** when you need a 911, triage, ambulance, disaster, search & rescue primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/emergency/911` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/emergency/911 \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/emergency/911", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/emergency/911", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/emergency/911"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/emergency/911");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/emergency/911");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/emergency/911")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/emergency/911",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/emergency/911", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/emergency/911
...
Every response from `/api/emergency/911` carries:
