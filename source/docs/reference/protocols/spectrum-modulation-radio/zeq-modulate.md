---
sidebar_position: 2
title: "ZeqModulate — Carrier Synchronization"
description: "QAM/OFDM modulation with HulyaPulse carrier sync. Symbol timing derived from Zeqond boundaries — receivers lock to the same phase without preamble overhead."
---
...
QAM/OFDM modulation with HulyaPulse carrier sync. Symbol timing derived from Zeqond boundaries — receivers lock to the same phase without preamble overhead. It belongs to the **Spectrum, modulation, radio** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/telecom/modulate`, Zeq runs the **ZeqModulate — Carrier Synchronization** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqModulate — Carrier Synchronization** when you need a spectrum, modulation, radio primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/telecom/modulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/telecom/modulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/telecom/modulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/telecom/modulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/telecom/modulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/telecom/modulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/telecom/modulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/telecom/modulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/telecom/modulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/telecom/modulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/telecom/modulate
...
Every response from `/api/telecom/modulate` carries:
