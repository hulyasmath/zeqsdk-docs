---
sidebar_position: 6
title: "ZeqPropagation — RF Path Loss"
description: "RF propagation modeling using electromagnetic operators. Free-space path loss, atmospheric absorption, and multipath fading computed from first principles."
---
...
RF propagation modeling using electromagnetic operators. Free-space path loss, atmospheric absorption, and multipath fading computed from first principles. It belongs to the **Spectrum, modulation, radio** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/telecom/propagation`, Zeq runs the **ZeqPropagation — RF Path Loss** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqPropagation — RF Path Loss** when you need a spectrum, modulation, radio primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/telecom/propagation` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/telecom/propagation \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/telecom/propagation", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/telecom/propagation", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/telecom/propagation"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/telecom/propagation");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/telecom/propagation");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/telecom/propagation")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/telecom/propagation",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/telecom/propagation", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/telecom/propagation
...
Every response from `/api/telecom/propagation` carries:
