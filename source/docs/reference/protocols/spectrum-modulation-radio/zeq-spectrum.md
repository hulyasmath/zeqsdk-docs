---
sidebar_position: 1
title: "ZeqSpectrum — Phase-Locked Allocation"
description: "Spectrum allocation using phase-locked frequency assignment. Frequencies assigned as harmonics of 1.287 Hz — no collisions when all transmitters sync to HulyaPulse."
---
...
| **Category** | Spectrum, modulation, radio |
| **Version** | 1.0 |
...
Spectrum allocation using phase-locked frequency assignment. Frequencies assigned as harmonics of 1.287 Hz — no collisions when all transmitters sync to HulyaPulse. It belongs to the **Spectrum, modulation, radio** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/telecom/spectrum/allocate`, Zeq runs the **ZeqSpectrum — Phase-Locked Allocation** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.

In practice, this protocol takes the parameters listed below, performs its spectrum, modulation, radio operation, and returns a structured response containing the computation output plus phase-locking metadata (`zeqondTick`, `hulyaPhase`, `zeqProof`).

## When to use it

Reach for **ZeqSpectrum — Phase-Locked Allocation** when you need a spectrum, modulation, radio primitive that:
...
If you only need a one-shot spectrum, modulation, radio answer with no audit trail and no composition with other Zeq calls, a plain library may be cheaper. If you need *any* of the four properties above, this protocol is the right tool.

## How to call it

The fastest path is a single HTTPS `POST` request to `/api/telecom/spectrum/allocate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/telecom/spectrum/allocate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/telecom/spectrum/allocate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/telecom/spectrum/allocate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/telecom/spectrum/allocate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/telecom/spectrum/allocate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/telecom/spectrum/allocate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/telecom/spectrum/allocate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/telecom/spectrum/allocate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/telecom/spectrum/allocate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/telecom/spectrum/allocate
...
Every response from `/api/telecom/spectrum/allocate` carries:
...
- All protocols in this family — see the [Spectrum, modulation, radio](/reference/protocols) category
