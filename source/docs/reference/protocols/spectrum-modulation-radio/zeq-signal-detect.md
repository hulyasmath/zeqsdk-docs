---
sidebar_position: 5
title: "ZeqDetect — Signal Detection"
description: "Weak signal detection using HulyaPulse phase correlation. Signals buried in noise become detectable when correlated against the known 1.287 Hz reference."
---
...
Weak signal detection using HulyaPulse phase correlation. Signals buried in noise become detectable when correlated against the known 1.287 Hz reference. It belongs to the **Spectrum, modulation, radio** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/telecom/detect`, Zeq runs the **ZeqDetect — Signal Detection** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqDetect — Signal Detection** when you need a spectrum, modulation, radio primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/telecom/detect` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/telecom/detect \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/telecom/detect", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/telecom/detect", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/telecom/detect"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/telecom/detect");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/telecom/detect");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/telecom/detect")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/telecom/detect",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/telecom/detect", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/telecom/detect
...
Every response from `/api/telecom/detect` carries:
