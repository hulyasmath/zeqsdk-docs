---
sidebar_position: 11
title: "ZeqFleet — Device Fleet Management"
description: "Fleet-wide IoT device management. Monitor health, sync status, and firmware across thousands of devices — all coordinated on the Zeqond grid."
---
...
Fleet-wide IoT device management. Monitor health, sync status, and firmware across thousands of devices — all coordinated on the Zeqond grid. It belongs to the **Power grids, solar, fusion** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/iot/fleet/status`, Zeq runs the **ZeqFleet — Device Fleet Management** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqFleet — Device Fleet Management** when you need a power grids, solar, fusion primitive that:
...
The fastest path is a single HTTPS `GET` request to `/api/iot/fleet/status` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X GET \
  https://www.zeq.dev/api/iot/fleet/status \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/iot/fleet/status", {
  method: "GET",
  headers: {
...
func main() {
    req, _ := http.NewRequest("GET", "https://www.zeq.dev/api/iot/fleet/status", nil)
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/iot/fleet/status"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/iot/fleet/status");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/iot/fleet/status");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/iot/fleet/status")!)
req.httpMethod = "GET"
...
http.request{
    url = "https://www.zeq.dev/api/iot/fleet/status",
    method = "GET",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/iot/fleet/status", {
  method: "GET",
  headers: {
...
```http
GET https://www.zeq.dev/api/iot/fleet/status
...
Every response from `/api/iot/fleet/status` carries:
