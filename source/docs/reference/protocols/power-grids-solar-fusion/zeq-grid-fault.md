---
sidebar_position: 6
title: "ZeqGrid Fault Detection"
description: "Grid fault detection using operator anomaly coupling. Phase deviation beyond tolerance triggers fault isolation — HulyaPulse provides the reference clock for fault timing."
---
...
Reach for **ZeqGrid Fault Detection** when you need a power grids, solar, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/energy/grid/fault` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/energy/grid/fault \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/energy/grid/fault", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/energy/grid/fault", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/energy/grid/fault"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/energy/grid/fault");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/energy/grid/fault");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/energy/grid/fault")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/energy/grid/fault",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/energy/grid/fault", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/energy/grid/fault
...
Every response from `/api/energy/grid/fault` carries:
