---
sidebar_position: 3
title: "ZeqFusion — Plasma Confinement"
description: "Fusion plasma confinement modeling using plasma physics operators. Tokamak equilibrium, MHD stability, and Lawson criterion calculation via the HULYAS solver."
---
...
Fusion plasma confinement modeling using plasma physics operators. Tokamak equilibrium, MHD stability, and Lawson criterion calculation via the HULYAS solver. It belongs to the **Power grids, solar, fusion** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/energy/fusion/simulate`, Zeq runs the **ZeqFusion — Plasma Confinement** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqFusion — Plasma Confinement** when you need a power grids, solar, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/energy/fusion/simulate` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/energy/fusion/simulate \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/energy/fusion/simulate", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/energy/fusion/simulate", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/energy/fusion/simulate"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/energy/fusion/simulate");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/energy/fusion/simulate");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/energy/fusion/simulate")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/energy/fusion/simulate",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/energy/fusion/simulate", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/energy/fusion/simulate
...
Every response from `/api/energy/fusion/simulate` carries:
