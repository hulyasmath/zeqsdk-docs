---
sidebar_position: 10
title: "ZeqFusion — Sensor Fusion"
description: "Multi-sensor data fusion using ZeqLattice coherence. Combine readings from diverse sensors — the framework weights them by phase alignment and measurement quality."
---
...
Reach for **ZeqFusion — Sensor Fusion** when you need a power grids, solar, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/iot/fusion` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/iot/fusion \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/iot/fusion", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/iot/fusion", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/iot/fusion"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/iot/fusion");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/iot/fusion");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/iot/fusion")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/iot/fusion",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/iot/fusion", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/iot/fusion
...
Every response from `/api/iot/fusion` carries:
