---
sidebar_position: 1
title: "ZeqGrid — Power Grid Balancing"
description: "Power grid load balancing using R(t) modulation. Demand response synchronized to HulyaPulse — loads shift to phase troughs, generation peaks match consumption peaks."
---
...
Reach for **ZeqGrid — Power Grid Balancing** when you need a power grids, solar, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/energy/grid/balance` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/energy/grid/balance \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/energy/grid/balance", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/energy/grid/balance", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/energy/grid/balance"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/energy/grid/balance");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/energy/grid/balance");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/energy/grid/balance")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/energy/grid/balance",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/energy/grid/balance", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/energy/grid/balance
...
Every response from `/api/energy/grid/balance` carries:
