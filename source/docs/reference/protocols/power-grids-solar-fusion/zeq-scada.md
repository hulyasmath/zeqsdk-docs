---
sidebar_position: 8
title: "ZeqSCADA — Secure Industrial Control"
description: "Secure industrial control using TESC ephemeral channels. Every command encrypted with a PLAT valid for one Zeqond (0.777s) — replay attacks physically impossible."
---
...
Reach for **ZeqSCADA — Secure Industrial Control** when you need a power grids, solar, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/iot/scada/command` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/iot/scada/command \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/iot/scada/command", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/iot/scada/command", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/iot/scada/command"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/iot/scada/command");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/iot/scada/command");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/iot/scada/command")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/iot/scada/command",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/iot/scada/command", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/iot/scada/command
...
Every response from `/api/iot/scada/command` carries:
