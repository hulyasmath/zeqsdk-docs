---
sidebar_position: 2
title: "ZeqSolar — Solar Prediction"
description: "Solar irradiance prediction using astrophysics operators + R(t) modulation. Forecasts energy output synced to Zeqond intervals."
---
...
Solar irradiance prediction using astrophysics operators + R(t) modulation. Forecasts energy output synced to Zeqond intervals. It belongs to the **Power grids, solar, fusion** family and is callable as a single REST endpoint, a one-line SDK call, or via streaming where applicable.

## What it does

When you call `/api/energy/solar/predict`, Zeq runs the **ZeqSolar — Solar Prediction** computation through the KO42 metric tensioner under the active HulyaPulse phase. The result is sealed at the next Zeqond boundary (0.777 s) and returned with a verifiable ZeqProof receipt — meaning the same inputs at the same phase always produce the same output, and any third party can later verify the result was computed at the time you claim.
...
Reach for **ZeqSolar — Solar Prediction** when you need a power grids, solar, fusion primitive that:
...
The fastest path is a single HTTPS `POST` request to `/api/energy/solar/predict` with a Bearer token. You can use cURL, JavaScript, Python, or any HTTP client — examples for all three are below. The response is JSON.
...
```bash
curl -X POST \
  https://www.zeq.dev/api/energy/solar/predict \
...
```javascript
const res = await fetch("https://www.zeq.dev/api/energy/solar/predict", {
  method: "POST",
  headers: {
...
func main() {
    ...
    req, _ := http.NewRequest("POST", "https://www.zeq.dev/api/energy/solar/predict", bytes.NewBuffer(payload))
...
public class ZeqCall {
  public static void main(String[] args) throws Exception {
    ...
    HttpRequest req = HttpRequest.newBuilder()
        .uri(URI.create("https://www.zeq.dev/api/energy/solar/predict"))
...
int main(void) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.zeq.dev/api/energy/solar/predict");
...
```php
<?php
$ch = curl_init("https://www.zeq.dev/api/energy/solar/predict");
...
var req = URLRequest(url: URL(string: "https://www.zeq.dev/api/energy/solar/predict")!)
req.httpMethod = "POST"
...
http.request{
    url = "https://www.zeq.dev/api/energy/solar/predict",
    method = "POST",
    headers = {
...
```html
<script>
fetch("https://www.zeq.dev/api/energy/solar/predict", {
  method: "POST",
  headers: {
...
```http
POST https://www.zeq.dev/api/energy/solar/predict
...
Every response from `/api/energy/solar/predict` carries:
