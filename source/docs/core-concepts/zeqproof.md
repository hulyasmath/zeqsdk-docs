...
## Understanding Cryptographic Verification (For Non-Experts)

### What Does "Cryptographically Signed" Mean?

Imagine you receive a package with:
1. A document (your computation result)
2. A wax seal with an official stamp (the signature)

The seal proves two things:
- **Authenticity**: Only the official Zeq infrastructure has the seal stamp
- **Integrity**: If someone opens the package and modifies the document, the seal breaks (becomes invalid)

**ZeqProof works the same way** but with math instead of wax seals.
