...
**Real-world analogy:**
Imagine a universal equation that describes all fluids, but with domain-specific parameters:
- Water: C_k values tuned for viscosity ~10^{−3} Pa·s
- Honey: C_k values tuned for viscosity ~10^0 Pa·s
- Mercury: C_k values tuned for viscosity ~10^{−3} Pa·s but different density

The base equation is the same; the domain parameters (C_k) change.
